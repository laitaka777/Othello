...
function FabriqueCommentaireSolitaireNouveauFormat(whichSolitaire:t_SolitaireRecNouveauFormat) : str255;
function FabriqueChainePositionSolitaireNouveauFormat(whichSolitaire:t_SolitaireRecNouveauFormat) : str255;
...
USES UnitNouveauFormat,UnitBaseNouveauFormat,UnitSolitaire,UnitAccesStructuresNouvFormat,MyFileSystemUtils,
     UnitRapport,UnitServicesMemoire,UnitMacExtras,SNStrings;
...
begin
  ...
  if codeErreur=0 then
    with entete do
      for i := 1 to 64 do
        nbSolitairesCetteProf[i] := MySwapLongint(nbSolitairesCetteProf[i]);
...
begin
  with theSolitaire do
		begin
		  annee           := MySwapInteger(annee);
		  nroTournoi      := MySwapInteger(nroTournoi);
		  nroJoueurNoir   := MySwapLongint(nroJoueurNoir);
		  nroJoueurBlanc  := MySwapLongint(nroJoueurBlanc);
		end;
	      ...
	      with entete do
	        if (TailleDuPlateau <> 8) | (EstUnFichierSolitaire <> 1) & (ProfondeurCalculTheorique <> 0) then
			      begin
			        ...
			        WritelnStringAndNumDansRapport('Réparation de '+nomFichier^+'  => codeErreur = ',codeErreur);
			      end;
end;
    ...
    begin
      ...
      case traitSolitaire of
		    1 {noir} :
		      begin
		        if scoreParfait = 0
		          then GetIndString(commentaire,TextesSolitairesID,3)
		          else GetIndString(commentaire,TextesSolitairesID,1);
		      end;
		    2 {blanc} :
		      begin
		        if scoreParfait = 0
		          then GetIndString(commentaire,TextesSolitairesID,4)
		          else GetIndString(commentaire,TextesSolitairesID,2);
		      end;
		    0 {otherwise} :
		      begin
		        SysBeep(0);
      ...
      if referencesCompletes & (scoreReel <> 0) 
        then commentaire := commentaire + nom + ' ' + NumEnString(scoreReel) + '-'
...
var s : str255;
begin
   CompilerPosition(PackedOthelloPositionToPlOth(whichSolitaire.position),s);
...
var tableNbSolitaires:t_tableNbSolitaires;
    ...
    temp : SInt32;
begin
  if (nbCasesMin < 0) then nbCasesMin := 0;
  if (nbCasesMin > 64) then nbCasesMin := 64;
  if (nbCasesMax < 0) then nbCasesMax := 0;
  ...
  if (InfosFichiersNouveauFormat.nbFichiers <= 0) then LecturePreparatoireDossierDatabase(volumeRefCassio);
...
begin
  with InfosFichiersNouveauFormat do
    begin
		  ...
		  if (nbFichiers <= 0) then LecturePreparatoireDossierDatabase(volumeRefCassio);
		  ...
		  for k := 0 to 64 do
		    begin
		      WriteStringAndNumDansRapport('nb_sol['+NumEnString(k)+'] = ',tableNbSolitaires[k]);
		      WritelnStringAndNumDansRapport('   cumule['+NumEnString(k)+'] = ',tableNbSolitairesCumules[k]);
		    end;
		    ...
		    repeat
			    ...
			    if (nbCasesVides >= 1) & (numeroFichier>=1) & (numeroFichier <= nbFichiers) & (numeroSolitaireDansFichier >= 1) &
			      ...
			      begin
			        ...
			        WritelnStringAndNumDansRapport('numeroFichier = ',numeroFichier);
			        WritelnStringAndNumDansRapport('numeroSolitaireDansFichier = ',numeroSolitaireDansFichier);}
			        
			        error := OuvreFichierNouveauFormat(numeroFichier);
			        {WritelnStringAndNumDansRapport('error = ',error);}
