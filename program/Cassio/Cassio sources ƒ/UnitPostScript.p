...
type
  ...
  TDashedLineRec = 
    packed record
     ...
     intervals:  array[0..5] of SignedByte;{points for drawing and not drawing dashes}
    end;
...
procedure SendPostscript(s : str255);
...
procedure MyDrawXString(s: str255; ctr: Point;just, flip: SInt16; rot: Fixed);
procedure DrawTranslatedString(s: str255; decX,decY : double_t);
...
USES UnitDiagramFforum;



var LastNumerateurDansSetLineWidth : SInt16; 
    LastDenominateurDansSetLineWidth : SInt16; 


procedure InitUnitPostscript;
begin
  LastNumerateurDansSetLineWidth := 1;
  LastDenominateurDansSetLineWidth := 1;
end;
...
begin
  ...
  PostScriptParcel := AllocateMemoryHdl(Length(s));
  MoveMemory(MakeMemoryPointer(ord4(@s[0])+1),PostScriptParcel^,Length(s));
  PicComment(PostScriptHandle,Length(s),PostScriptParcel);
...
begin
  ...
  if DoitGenererPostScriptCompatibleXPress() &
    (LastNumerateurDansSetLineWidth <> 0) & (LastDenominateurDansSetLineWidth <> 0) then
    begin  {on multiplie la taille du crayon par l'inverse pour revenir ˆ (1,1) }
      lfh^^.num := LastDenominateurDansSetLineWidth;
      lfh^^.denom := LastNumerateurDansSetLineWidth;
      picComment(SetLineWidth,sizeof(linefactor),Handle(lfh));
    end;
  lfh^^.num := numerateur;
  lfh^^.denom := denominateur;
  picComment(SetLineWidth,sizeof(linefactor),Handle(lfh));
  DisposeMemoryHdl(Handle(lfh));
  LastNumerateurDansSetLineWidth := numerateur;
  LastDenominateurDansSetLineWidth := denominateur;
end;
...
begin
  ...
  s := s+s1+StringOf('.');
  unreel := 10*(unreel-MyTrunc(unreel));
  NumToString(MyTrunc(unreel),s1);
  s := s+s1;
  unreel := 10*(unreel-MyTrunc(unreel));
  NumToString(MyTrunc(unreel),s1);
...
procedure DrawTranslatedPostScriptString(s: str255; decX,decY : double_t);
var
  hT:         TTxtPicHdl;
  hC:         TCenterHdl;
  zeroRect:   Rect;
...
begin
  ...
  if (rInfo = NIL) or (rCenter = NIL)
     then DebugStr('AllocateMemoryHdl failed');
  with rInfo^^ do
  begin
     rFlip := 0;
     rAngle := - angle;
