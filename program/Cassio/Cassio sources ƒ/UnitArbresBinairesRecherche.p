UNIT UnitArbresBinairesRecherche;
...
USES MacTypes, UnitPagesABR;

type ABRProc=procedure(var x : ABR);
...
procedure ABRAffichageInfixe(nomABR : str255;x : ABR);
procedure ABRAffichagePrefixe(nomABR : str255;x : ABR);
procedure ABRAffichageSuffixe(nomABR : str255;x : ABR);
...
USES UnitServicesMemoire,UnitRapport,MyMathUtils;
...
begin
  if x = NIL
    then 
      ...
      begin
        WriteStringAndNumDansRapport('cle=',x^.cle);
        WritelnStringAndNumDansRapport('  data=',x^.data);
      end;
end;
...
begin
  while (x <> NIL) do
    begin
      comp := ABRCompareClefs(whichKey,x^.cle);
      if comp=0
        then 
          begin
            ABRSearch := x;
