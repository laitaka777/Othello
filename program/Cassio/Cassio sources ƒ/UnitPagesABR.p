...
USES UnitOth1,UnitServicesDialogs,MacTypes,UnitServicesMemoire,UnitRapport;
...
begin
  for i := 1 to nbPagesDeABR do
	...
	if PeutCreerNouvellePageABR() then;
	if PeutCreerNouvellePageABR() then;
	if PeutCreerNouvellePageABR() then;
	if PeutCreerNouvellePageABR() then;
end;
...
begin
  ...
  WritelnStringAndNumDansRapport('soldeCreationABR=',soldeCreationABR);
  SysBeep(0);
  AttendFrappeClavier;
  }
...
begin
  ...
  {WritelnStringAndNumDansRapport('appel de DisposeABRPaginee pour @',SInt32(G));}
