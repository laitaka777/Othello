...
USES UnitOth0,UnitAccesStructuresNouvFormat,UnitListe,UnitRapport,SNStrings;
...
begin
  ...
  if afficheInfos then
    begin
      ...
      WritelnStringAndNumDansRapport('nro de Noir = ',noir);
      WritelnStringAndNumDansRapport('nro de Blanc = ',blanc);
    end;
  
  anneeDebut  := GetAnneePremierePartieDeCeJoueur(noir);
  
  if afficheInfos then WritelnStringAndNumDansRapport('anneeDebut de Noir = ',anneeDebut);
  ...
  if afficheInfos then WritelnStringAndNumDansRapport('anneeArret de Noir = ',anneeArret);
  ...
  if afficheInfos then WritelnStringAndNumDansRapport('anneeDebut de Blanc = ',anneeDebut);
  ...
  if afficheInfos then WritelnStringAndNumDansRapport('anneeArret de Blanc = ',anneeArret);
...
begin
  ...
  WritelnStringAndNumDansRapport('nbPartiesPrisesEnComptePourLeClassement = ',nbPartiesPrisesEnComptePourLeClassement);
        ...
        then
	        begin
	          {if Pos('Busut',GetNomJoueur(i)) > 0 then 
		          begin
		            WritelnStringAndNumDansRapport(GetNomJoueur(i)+ ' : actif , nro = ',i);
		            WritelnStringAndNumDansRapport('annee debut de ce joueur = ',GetAnneePremierePartieDeCeJoueur(i));
		          end;}
		      ...
		      begin
		        {if Pos('Busut',GetNomJoueur(i)) > 0 then 
		          begin
		            WritelnStringAndNumDansRapport(GetNomJoueur(i)+ ' : inactif , nro = ',i);
		            WritelnStringAndNumDansRapport('annee debut de ce joueur = ',GetAnneePremierePartieDeCeJoueur(i));
		          end;}
		      end;
    end;
