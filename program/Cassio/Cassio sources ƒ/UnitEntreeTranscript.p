...
USES MacTypes,UnitPositionEtTrait,UnitArbresBinairesRecherche;
...
type Transcript =
        record
          chiffres : packed array[0..99,kGauche..kDroite] of signedByte;
          curseur : record
...
procedure RestoreReglagesAvantEditionTranscript;
...
function NombreCoupsCorrectDansTranscript(var analyse : AnalyseDeTranscript; var positionEtPartie : str255) : SInt16; 
...
procedure EcritMessageSousTranscript(message : str255);
...
procedure PlaquerPositionEtPartieLegaleDansOthellierDeGauche(position : PositionEtTraitRec;partiePlaquee : str255);
...
function ActionDeCorrectionEnString(whichAction : ActionDeCorrection) : str255;
...
USES UnitOth0,UnitScannerOthellistique,UnitBufferedPICT,UnitRapport,MyStrings,UnitActions,UnitArbreDeJeuCourant,
     UnitNormalisation,UnitNotesSurCases,UnitFenetres,UnitUtilitaires,MyUtils,UnitHashing,UnitStringSet,
     UnitTroisiemeDimension,UnitPhasesPartie,UnitJeu,UnitSetUp,UnitActions,UnitSaisiePartie,UnitOth1,
     UnitBufferedPICT,UnitDialog,UnitGestionDuTemps,UnitOth2,UnitVieilOthelliste,UnitIconisation,SNStrings,
     UnitCouleur;
...
const
      ...
      kMaxNiveauRecursionCorrectionAutomatique = 5;     { 5 corrections elementaires }
      kNombreMaxSuggestionsDeCorrection        = 12;    { 12 suggestions affichees}
      kTailleMaxArbreCorrectionAutomatique     = 5000;  { 5000 noeuds }
      kTailleMaxAppelsCorrectionAutomatique    = 10000; { 10000 appels recursifs }
...
procedure DebugEntreeTranscript(fonctionAppelante : str255);
begin
  WritelnStringAndNumDansRapport(fonctionAppelante+' : nroDernierCoupAtteint = ',nroDernierCoupAtteint);
  WritelnStringAndNumDansRapport(fonctionAppelante+' : aQuiDeJouer = ',aQuiDeJouer);
  WritelnStringAndNumDansRapport(fonctionAppelante+' : nbreCoup = ',nbreCoup);
end;
...
begin
  with gReglagesAvantEditionTranscript do
    begin
      BeginDrawingForTranscript;
      SizeWindow(wPlateauPtr,LoWrd(tailleFenetrePlateau),HiWrd(tailleFenetrePlateau),true);
        ...
        if not(HumCtreHum) & (aQuiDeJouer <> couleurMacintosh) then
            begin
              if PeutArreterAnalyseRetrograde() then 
                LanceInterruption(InterruptionSimple,'PrepareCassioPourEditionTranscript');
            end;
...
begin
  if EnModeEntreeTranscript()
    then
      begin
        gEnModeEntreeTranscript := false;
        RestoreReglagesAvantEditionTranscript;
      end
    else
      begin
        if not(analyseRetrograde.enCours) then
...
procedure RaiseTranscriptError(whichError : SInt32;square : SInt16; var analyse : AnalyseDeTranscript);
begin
  {WritelnStringAndNumDansRapport('Raising transcript error for square '+CoupEnString(square,true)+', error : ',whichError);}
      ...
      if (square >= 11) & (square <= 88) 
        then 
          ...
          if (whichError <> kTranscriptCoupManquant)
            then WritelnStringAndNumDansRapport('ERROR ! square out of range in RaiseTranscriptError : ', square);
    end;
end;
...
function CaseAyantLePlusPetitNumeroAuDela(numeroMinimum : SInt16) : SInt16; 
var t,valeur,max,result : SInt16; 
    myTranscript : Transcript;
begin
  myTranscript := CurrentTranscript();
  with myTranscript do
      ...
      for t := 11 to 88 do
        if EstUneCaseVideDansTranscript(t,myTranscript) then
          begin
            ...
            if (valeur >= numeroMinimum) & (valeur <= max) then
              begin
                max := valeur;
                result := t;
              end;
...
var test : boolean;
begin
  test := SamePositionEtTrait(PositionEtTraitInitiauxDuTranscriptCourant(),GetPositionEtTraitInitiauxOfGameTree()) & 
...
begin
  ...
  if (square1 >= 11) & (square1 <= 88) &
    ...
    begin
      ...
      if not(gCouleurOthellier.estUneTexture) & not(retirerEffet3DSubtilOthellier2D) then
        begin
...
begin
  ...
  (* FIXME !! sale hack : on utilise les routines qui 
...
begin
  ...
  with analyse do
    begin
        ...
        if EstUneCaseVideDansTranscript(derniereCaseTapee,myTranscript) &
           ...
           begin
             if (TranscriptALaPositionDeDepartInversee(myTranscript) & SquareInSquareSet(derniereCaseTapee,[34,43,56,65])) |
               ...
               then InverserCouleurDesPionsDuTranscript(myTranscript);
           end;
      ...
      nombreCoupsPossibles := NombreCoupsCorrectDansTranscript(analyse,plusLonguePartieLegale);
...
begin
  ...
  jusqueQuelCoup := Length(partiePlaquee) div 2;
      ...
      if (nbreCoup > 0) & IsPrefix(partiePlaquee,partieDeCassioCourante)  
        then
          begin
            ...
            RejouePartieOthelloFictive(partiePlaquee,jusqueQuelCoup,SamePositionEtTrait(position,PositionEtTraitInitiauxStandard()),
                                       position.position,GetTraitOfPosition(position),gameNodeLePlusProfondGenere,kPeutDetruireArbreDeJeu);
          ...
          begin
            ...
            if SamePositionEtTrait(GetPositionInitialePartieEnCours(),position)
              then DoDebut(false)
    ...
    if (nombreCoupsPossibles > 0) then
      begin
        ...
        if DoitAfficherLaPlusLonguePartiePossible() then
          begin
            if SamePositionEtTrait(position,GetPositionEtTraitInitiauxOfGameTree()) & 
...
begin
  ...
  positionEtPartie := s;
  NombreCoupsCorrectDansTranscript := compteur;
end;
...
begin
  ...
  with gAnalyseDuTranscript do
    begin
      ...
      if (BitAnd(whichErrors,kTranscriptCoupManquant) <> 0) & CurrentTranscriptHasThisErrors(kTranscriptCoupManquant)
          ...
          begin
            ...
            if gameOver then
              begin
                ...
                if not(positionFeerique) then DialogueSaisiePartie;
              end;
          end
        else  
          begin
...
function MakeTranscriptFromLegalGame(legalGame : str255; const myTranscript : Transcript) : Transcript;
...
begin
  ...
  if EstUnePartieOthello(legalGame,true) then 
    begin
      longueur := Length(legalGame) div 2;
      k := 1;
      while (k <= longueur) do
        begin
...
begin
  ...
  if (ch=chr(FlecheHautKey))     | 
    ...
    begin
      ...
      case ord(ch) of
        ...
        FlecheGaucheKey    : if command
                               then BougerCurseurDebutDeLigne(gPileTranscripts.pile[gPileTranscripts.current])
                               else 
                                 if shift
        ...
        FlecheDroiteKey    : if command
                               then BougerCurseurFinDeLigne(gPileTranscripts.pile[gPileTranscripts.current])
                               else 
                                 if shift
        ...
        '+' : begin
                ...
                if (gAnalyseDuTranscript.cases[n+1].cardinal = 0) 
                  then myTranscript := IncrementerCetteCase(GetCurrentSquareOfCurseur(), +1, CurrentTranscript())
                    ...
                    begin
                      if (gAnalyseDuTranscript.cases[n+1].cardinal > 1) | shift
                        then decalage := 2
                        else decalage := 1;
  ...
  {WritelnStringAndNumDansRapport('ord(ch) = ',ord(ch));}
...
var profondeurStockee : SInt32;
    ...
    decalage,placeDisponible : SInt32;
            ...
            if (distanceRacine < profondeurArret) &
              ...
              with analyse^ do
                begin
                        ...
                        begin
                          if not(HasTheseErrorsForThisMove(kTranscriptCoupsDupliques,analyse^,n)) then
                            begin
                              ...
                              { transposition avec les coups n-1 et n-2 ? }
                              n1 := n;
                              ...
                              { transposition avec les coups n+10, n+20, etc. ? }      
                              n1 := n;
                              for c1 := 1 to cases[n1].cardinal do
                                for k := 1 to 4 do
                                  begin
                  ...
                  (* doublons : on essaie (a) de renvoyer les coups en doubles vers des coups manquants
                  ...
                  if HasTranscriptErrorMask(kTranscriptCoupsDupliques,analyse^) then
                    begin
                      ...
                      decalage := NombreDoublonsConsecutifsApresCeCoup(n,analyse^);
            ...
            begin
              ...
              if (analyseDepart <> NIL) then
                begin
                  ...
                  if (NombreErreurs(analyseDepart^) <= kMaxNiveauRecursionCorrectionAutomatique + 2)
                    ...
                    begin
                      for profondeurDeRecherche := 1 to kMaxNiveauRecursionCorrectionAutomatique do
                        RechercheRecursiveDesCorrections(whichTranscript,0,profondeurDeRecherche);
                    end;
                
                end;
                
              (* WritelnStringAndNumDansRapport('searchHistory.cardinal = ',searchHistory.cardinal);
                 WritelnStringAndNumDansRapport('gTranscriptSearch.nbreNoeudsVisites = ',gTranscriptSearch.nbreNoeudsVisites);
                 WritelnStringAndNumDansRapport('temps = ',TickCount() - ticks); *)
