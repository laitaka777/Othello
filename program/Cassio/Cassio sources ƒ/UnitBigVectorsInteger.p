...
USES MacTypes,UnitFichiersTEXT,UnitBigVectors;


TYPE

  TypeIntegerArray = array[0..0] of SInt16;
  TypeIntegerArrayPtr= ^TypeIntegerArray;
  
  
  PointMultidimensionnelInteger = 
                           record
                             taille : SInt32;
                             data:TypeIntegerArrayPtr;
                             alloue : boolean;
                           end;
...
USES fp,UnitOth1,UnitRapport,UnitServicesMemoire;
...
begin
  ...
  count := SizeOf(SInt16);
  count := (n+1)*count;
  p.data := TypeIntegerArrayPtr(AllocateMemoryPtr(count));
  if p.data <> NIL
    then
      begin
        p.taille := n;
    ...
    then DisposeMemoryPtr(Ptr(p.data))
      ...
      begin
        ...
        if not(p.alloue)
          then 
            begin
              WritelnStringAndNumDansRapport('DisposePointMultidimensionnelInteger : pointeur non alloue : ',SInt32(p.data));
