...
USES UnitRapport,UnitMacExtras;
...
begin
  if (prof < k_MIN_PROF_MINIPROFILER) | (prof > k_MAX_PROF_MINIPROFILER) then
    begin
      WritelnStringAndNumDansRapport('ERROR : prof out of range dans AjouterTempsDansMiniProfiler!  prof = ',prof);
      exit(AjouterTempsDansMiniProfiler);
    end;
  
  if (nbVides < 0) | (nbVides > 64) then
    begin
      WritelnStringAndNumDansRapport('ERROR : nbVides out of range dans AjouterTempsDansMiniProfiler! nbVides = ',nbVides);
      exit(AjouterTempsDansMiniProfiler);
    end;
...
procedure AfficheMiniProfilerDansRapport(affichage : SInt32);
var tempsTotal,tempsLocal : extended;
    i,j : SInt32;
    remplie : boolean;
begin
    ...
    begin
      ...
      for i := 0 to 64 do
        begin
          ...
          if remplie then
            begin
              WriteStringAndNumDansRapport(' prof = ',i);
              for j := k_MIN_PROF_MINIPROFILER to k_MAX_PROF_MINIPROFILER do
                if (occu[i,j] > 0) then
	                begin
	                  tempsLocal := MicrosecondesToSecondes(temps[i,j]);
	                  
	                  WriteStringAndNumDansRapport('  (j = ',j);
	                  WriteStringAndNumDansRapport(',occ = ',occu[i,j]);
