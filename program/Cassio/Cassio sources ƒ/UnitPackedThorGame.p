...
USES MacTypes,StringTypes,UnitDefinitionsPackedThorGame;
...
{$ifc not packed_thor_accessors_are_macros}
function GET_NTH_MOVE_OF_PACKED_GAME(const whichGame : PackedThorGame; n : UInt8; fonctionAppelante : str255) : UInt8; 
...
procedure COPY_STR60_TO_PACKED_GAME(const s : str60; var result : PackedThorGame);
procedure COPY_PACKED_GAME_TO_STR60(const game : PackedThorGame; var result : str60);
procedure COPY_PACKED_GAME_TO_STR255(const game : PackedThorGame; var result : str255);
...
procedure WRITELN_PACKED_GAME_DANS_RAPPORT(message : str255; whichGame : PackedThorGame);
...
USES UnitServicesMemoire,MyMathUtils,UnitRapport,SNStrings;
...
begin
  ...
  if (whichGame.theMoves[0] < 0) | (whichGame.theMoves[0] > 60) then
    begin
      ...
      WritelnStringAndNumDansRapport('@=', SInt32(@whichGame));
      WritelnStringAndNumDansRapport('whichGame.theMoves[0] = ',whichGame.theMoves[0]);
...
begin
  ...
  exceptionDeSentinelleConnue := ((n = 1) & (whichGame.theMoves[n] = 197)) |
                                 ...
                                 ((whichGame.theMoves[0] = 60) & (whichGame.theMoves[n] = 0) & (n > 0) & (n <= 60));
  ...
  if ((whichGame.theMoves[n] < 11) | (whichGame.theMoves[n] > 88)) & not(exceptionDeSentinelleConnue)
    ...
    begin
      ...
      WritelnStringAndNumDansRapport('@=', SInt32(@whichGame));
      WritelnStringAndNumDansRapport('n = ',n);
      WritelnStringAndNumDansRapport('whichGame.theMoves[n] = ',whichGame.theMoves[n]);
      WRITELN_PACKED_GAME_DANS_RAPPORT('whichGame = ',whichGame);
    end;
...
begin
  ...
  if (n < 1) | (n > 60) then
    begin
      WritelnStringAndNumDansRapport('WARNING : n out of range dans SET_NTH_MOVE_OF_PACKED_GAME  !! @=', SInt32(@whichGame));
      WritelnStringAndNumDansRapport('n = ',n);
      WRITELN_PACKED_GAME_DANS_RAPPORT('whichGame = ',whichGame);
    end;
...
procedure COPY_STR60_TO_PACKED_GAME(const s : str60; var result : PackedThorGame);
begin
  MoveMemory(@s[0], @result.theMoves[0], ord(s[0]) + 1);
end;


procedure COPY_PACKED_GAME_TO_STR60(const game : PackedThorGame; var result : str60);
begin
  MoveMemory(@game.theMoves[0], @result[0], game.theMoves[0] + 1);
end;
...
begin
  ...
  WriteStringAndNumDansRapport(' (len = ',len);
  WritelnStringDansRapport(' )');
  for i := 1 to len do
    begin
      coup := whichGame.theMoves[i];
      WriteStringAndNumDansRapport(NumEnString(i) + ':',i);
      WriteStringAndNumDansRapport(' => ',coup);
