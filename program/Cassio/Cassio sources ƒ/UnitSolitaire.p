...
function EstUnSolitaire(var meilleurX,bstdef : SInt32;couleur,MFprof,nbBl,nbNo : SInt32;
                        var jeu : plateauOthello; var empl : plBool; var frontiere : InfoFrontRec;
                        var score,nbreMeilleurs : SInt32;PourVraimentJouer : boolean;
                        var causeRejet : SInt32;traitement,scoreaatteindre : SInt32) : boolean;

procedure PlaquerSolitaire(PositionEtCommentaire : str255);
...
USES UnitUtilitairesFinale,Unit_AB_Simple,UnitSetUp,UnitFenetres,UnitPhasesPartie,UnitScannerOthellistique,
     UnitSuperviseur,UnitStrategie,UnitActions,UnitPackedOthelloPosition,UnitFinalePourPositionEtTrait,
     UnitStatistiqueDeSolitaire,UnitRapportImplementation,UnitCarbonisation,UnitUtilitaires,
     UnitOth1,UnitRapport,UnitJaponais,UnitServicesDialogs,UnitMoveRecords,UnitOth2,UnitAffichageReflexion,
     UnitSolitairesNouveauFormat,MyStrings,UnitPackedOthelloPosition,UnitDialog,UnitGestionDuTemps,SNStrings,
     UnitListe,UnitMenus,UnitCourbe,UnitNormalisation;
...
begin
  ...
  WritelnStringAndNumDansRapport('MFprof=',MFprof);
  WritelnStringAndNumDansRapport('nbBl=',nbBl);
  WritelnStringAndNumDansRapport('nbNo=',nbNo);
  WritelnPositionEtTraitDansRapport(jeu,couleur);
  WritelnStringAndBooleenDansRapport('PourVraimentJouer=',PourVraimentJouer);
  WritelnStringAndNumDansRapport('traitement=',traitement);
  WritelnStringAndNumDansRapport('scoreaatteindre=',scoreaatteindre);
 end;
...
if interruptionReflexion = pasdinterruption then   
 begin
   ...
   inc(nbreAppelsABSol);
   (*
   WriteStringAndNumAt('--> ABSol Petite         ',nbreAppelsABSol,10,150);
...
if (interruptionReflexion = pasdinterruption) then   
 begin    
   ...
   *)
   ...
   inc(nbreAppelsABSol);
   (*EssaieSetPortWindowPlateau;
   WriteStringAndNumAt('--> ABSol         ',nbreAppelsABSol,10,150);
   WriteStringAndNumAt('nbCoinsVides=',nbCoinsVides,10,160);
   WriteStringAndNumAt('gNbreCasesVidesSansCoinsEntreeSolitaire=',gNbreCasesVidesSansCoinsEntreeSolitaire,10,170);
   WriteStringAndNumAt('nbCasesVides=',nbCasesVides,10,180);*)
   
   FilsDePere := fils[pere];
   (*
   WriteStringAndNumAt('pere=',pere,100,190);
   WriteStringAndNumAt('FilsDePere=',FilsDePere,10,190);
   ...
   *)
   ...
   BEGIN
    iCourant := FilsDePere;
    (* WriteStringAndNumAt('test de filsdepere=',filsdepere,10,10);AttendFrappeClavier; *)
    if plat[iCourant] = pionVide then 
      begin
        inc(nbVidesTrouvees);
        if ModifPlatFin(iCourant,couleur,platEssai,nbBlcEssai,nbNrEssai) then 
    ...
    *)
    ...
    (*
    WriteStringAndNumAt('test (dans petite boucle) de iCourant=',iCourant,10,10);
    WriteStringAndNumAt('index=',index,10,20);
    AttendFrappeClavier;
    *)
   ...
   begin
    iCourant := coinsvides[index];
    WriteStringAndNumAt('test (dans grande boucle) de iCourant=',iCourant,10,10);
    WriteStringAndNumAt('index=',index,10,20);
    AttendFrappeClavier;
   ...
   *)
   ...
   BEGIN
    ...
    (*
    EssaieSetPortWindowPlateau;
    WriteStringAndNumAt('test (dans gNbreCasesVidesSansCoinsEntreeSolitaire) de iCourant=',iCourant,10,10);
    WriteStringAndNumAt('index=',index,10,20);
    AttendFrappeClavier;
    *)
    if iCourant<>filsDePere then
  ...
  if not(Ajoue) then 
      ...
      (*
        ...
        WriteStringAndNumAt('nbVidesTrouvees=',nbVidesTrouvees,10,200);
...
if (interruptionReflexion = pasdinterruption) then   
 begin    
   ...
   *)
   ...
   inc(nbreAppelsABSol);
   (*EssaieSetPortWindowPlateau;
   WriteStringAndNumAt('--> AlphaBetaSolitaireFast         ',nbreAppelsABSol,10,150);
   WriteStringAndNumAt('nbCoinsVides=',nbCoinsVides,10,160);
   WriteStringAndNumAt('gNbreCasesVidesSansCoinsEntreeSolitaire=',gNbreCasesVidesSansCoinsEntreeSolitaire,10,170);
   WriteStringAndNumAt('nbCasesVides=',nbCasesVides,10,180);*)
   ...
   if utiliseMilieuDePartie
     then
       nbCoupsPourCoul := TrierSelonDivergenceAvecMilieu(plat,couleur,nbVidesTrouvees,conseilHash,listeCasesVides,listeCasesVides,infosMilieuDePartie,
                                                         100*alpha,100*beta,probableFailLow,probableFailHigh,utiliseMilieuDePartie)
     else
   ...
   for i := 0 to 15 do
     for j := 0 to 15 do
       if (i=index1EnCours) & (j=index2EnCours)
         then WriteStringAndNumAt('*',tableauComparaison[i,j],10+i*50,10+j*15)
         else WriteStringAndNumAt(' ',tableauComparaison[i,j],10+i*50,10+j*15);
 end;
...
if (interruptionReflexion = pasdinterruption) then   
 begin 
  ...
  {if couleurmini = pionNoir 
    then WriteStringAndNumAt('pionNoir et prof=' , MiniProf, 10, 120)
    else WriteStringAndNumAt('pionBlanc et prof=' , MiniProf, 10, 120);}
  ...
  repeat
    ...
    if MiniProf<=0 then
      ...
      begin
        ...
        WritelnStringAndNumDansRapport('tempsComparaisonNormal = ',tempsComparaisonFast);
        ...
        *)
        ...
        (*
        WritelnStringAndNumDansRapport('profPourTriSelonDivergenceDansSolitaire = ',profPourTriSelonDivergenceDansSolitaire);
        WritelnStringAndNumDansRapport('profMinimalePourUtilisationMilieuSolitaire = ',profMinimalePourUtilisationMilieuSolitaire);
        WritelnStringAndNumDansRapport('valXYFast = ',valXY);
        WritelnStringAndBooleenDansRapport('test solitaire fast = ',testsolitaire);
        WritelnStringAndNumDansRapport('tempsComparaisonFast = ',tempsComparaisonFast);
        WritelnDansRapport('');
        ...
        *)
        ...
        begin
          ...
          WriteStringAndNumAt('',xcourant,200-couleurmini*130,200+12*compteur);
          WriteStringAndNumAt(',',defense,220-couleurmini*130,200+12*compteur);
          WriteStringAndNumAt('==>',valXY,260-couleurmini*130,200+12*compteur);
          WriteStringAndNumAt('                                       ',0,300-couleurmini*130,200+12*compteur);
          if testsolitaire 
             then WriteStringAndNumAt('solitaire (nb def: ',nbreDefensesMod,300-couleurmini*130,200+12*compteur)
             else WriteStringAndNumAt('nb def : ',nbreDefensesMod,300-couleurmini*130,200+12*compteur);
          WriteStringAndNumAt('                                             ',0,200-couleurmini*130,200+12*compteur+12);
          SetPort(oldport);
        end;
    ...
    SetValReflexFinale(classAux,miniprof,compteur,longClass,reflParfait,nroCoupRecherche,compteur+1,couleur);
		if affichageReflexion.doitAfficher then LanceDemandeAffichageReflexion(true);
...
begin
   ...
   if (killing = NIL) | (killer = NIL) then
     begin
       InitMaterielSolitaire := -1;
       exit(InitMaterielSolitaire);
     end;

   MemoryFillChar(killing,sizeof(killing),chr(0));
   MemoryFillChar(killer,sizeof(killer),chr(0));
...
begin      
   ...
   if not(HumCtreHum) & pourVraimentJouer then
     begin
       ...
       if commentaire<>'' then
         begin
           GetIndString(s,TextesSolitairesID,6);  {annule}
     ...
     begin
      ...
      if (interruptionReflexion = pasdinterruption) then 
        begin
                   ...
                   PeutJouerIci(-couleur,iCourant,jeu) 
                    then 
                      begin
                        ...
                        {
                        SysBeep(30);
                        WriteStringAndNumAt('il faut jouer en ',iCourant,30,60);
                        }
                      end;
          ...
          SetValReflex(classement,MFniv,nbCoup,nbCoup,ReflAnnonceParfait,nroCoupRecherche,MaxInt,couleur);
          if affichageReflexion.doitAfficher then LanceDemandeAffichageReflexion(false);
    ...
    if HumainPeutJouer(plat) then
                  ...
                  then 
                    ...
                    begin
                      if HumainPeutJouer(platEssai)
                        then 
                          begin
                            GetIndString(s,TextesSolitairesID,7);
      ...
      begin
        if OrdiPeutJouer(plat) 
         then
          begin
            GetIndString(s,TextesSolitairesID,8);
            erreurES := WriteDansFichierTexte(fichierSolution,StringOf(' ')+s);
...
begin
  ...
  commentaire := CommentaireSolitaire^^;
  GetIndString(s,TextesSolitairesID,5);
  ...
  if HumainPeutJouer(position) 
    then 
      begin
        reply.fname := nomprec;
        BeginDialog;
        GetIndString(s,TextesSolitairesID,9);
        if MakeFileName(reply,s,mySpec) then;
        EndDialog;
        if reply.good then
          begin
            GetPort(oldport);
            ...
            FenetreMessage := NewCWindow(NIL,unRect,'',false,1,FenetreFictiveAvantPlan(),false,0);
            if FenetreMessage <> NIL then
              begin
                ...
                TextSize(0);
                Moveto(50,21);
                GetIndString(s,TextesSolitairesID,10);
                DrawString(s);
                Moveto(50,38);
                GetIndString(s,TextesSolitairesID,11);
                DrawString(s);
                TextFont(gCassioApplicationFont);
                TextSize(gCassioSmallFontSize);
                Moveto(3,82);
                GetIndString(s,TextesSolitairesID,12);
                DrawString(s);
                for i := 0 to 4 do
                  begin
                    Moveto(70+i*100,67);
                    ...
                    if i=0
                      then WriteStringAndNumAt('',i*100,70+i*100-2,66)
                      else WriteStringAndNumAt('',i*100,70+i*100-9,66);
                  end;
                Moveto(70,73);
                Lineto(70+4*100,73);
            ...
            if erreurES=NoErr then 
			      ...
			      if erreurES <> 0 
			        then AlerteSimpleFichierTexte(reply.fName,erreurES)
			        else
			          begin
            ...
            SetFileCreatorFichierTexte(fichierSolution,'R*ch'); 
            SetFileTypeFichierTexte(fichierSolution,'TEXT');
      ...
      begin
        {WriteStringAndNumAt('LE JOUEUR HUMAIN NE PEUT PAS JOUER !!!',0,10,100);}
        BeginDialog;
        dp := MyGetNewDialog(PasPossibleSolitaireID,FenetreFictiveAvantPlan());
    ...
    case traitement of
      ...
      kSortiePapierLongue:
        begin
         ...
         if not(CassioEstEnModeSolitaire())
           then 
             begin
               GetIndString(s,TextesSolitairesID,13);
               AlerteSimple(s);
             end
           else
             begin
               GetIndString(nomprec,TextesSolitairesID,14);
               ...
               if (nbrelignesSolution>0) & not(InterruptionCalculSuites) then
                if (nbrelignesSolution<80) & (traitement<>kSortiePapierLongue) 
                 then 
                   begin
                     NumToString(nbrelignesSolution,s1);
                     GetIndString(s,TextesSolitairesID,15);
                     ParamText(ParamStr(s,s1,ReelEnStringAvecDecimales(DifficulteDuSolitaire(),3),'',''),'','','');
                     BeginDialog;
                     item := MyAlert(AlerteOuiNonID,FiltreClassiqueAlerteUPP,[BoutonOui,BoutonNon]);
                     EndDialog;
                     if item=BoutonOui then
                       begin
                         ...
                         if (nbrelignesSolution>0) & not(InterruptionCalculSuites) then
                           begin
                             NumToString(nbrelignesSolution,s1);
                             GetIndString(s,TextesSolitairesID,16);
                             BeginDialog;
                   ...
                   begin
                     if nbrelignesSolution>=MaxNbreLignesAutorisees
                       then 
                         begin
                           GetIndString(s,TextesSolitairesID,17);
                         end
                       else
                         begin
                           NumToString(nbrelignesSolution,s1);
     ...
     kRechercheSolitairesDansBase:
       begin
        ...
        nbCoupsNonPerdants := NbCoupsGagnantsOuNuls(MakePositionEtTrait(jeu,couleur),scoreWLDOptimal,2);
...
var couleur,t,i : SInt32;
    ...
    numeroDeCoup : SInt32;
    commentaire : str255;
    promptEnGras,resteDuCommentaire : str255;
begin
  ...
  GetIndString(s,TextesSolitairesID,20);
  ...
  if couleurMacintosh=couleur then
    begin
      ...
      LanceInterruption(interruptionSimple,'PlaquerSolitaire');
  ...
  commentaire := TPCopy(PositionEtCommentaire,22,Length(PositionEtCommentaire)-21);
  CommentaireSolitaire^^ := commentaire;
  GetIndString(s,TextesSolitairesID,1);
  if Pos(s,commentaire)>0  then ParamDiagPositionFFORUM.commentPositionFFORUM^^ := s;
  GetIndString(s,TextesSolitairesID,2);
  if Pos(s,commentaire)>0  then ParamDiagPositionFFORUM.commentPositionFFORUM^^ := s;
  GetIndString(s,TextesSolitairesID,3);
  if Pos(s,commentaire)>0  then ParamDiagPositionFFORUM.commentPositionFFORUM^^ := s;
  GetIndString(s,TextesSolitairesID,4);
  if Pos(s,commentaire)>0  then ParamDiagPositionFFORUM.commentPositionFFORUM^^ := s;    
...
begin
  ...
  err := ReadBufferDansFichierTexte(BaseSolitairesFic,MakeMemoryPointer(Partiebuff),count);
...
procedure LitJoueurNro(n : SInt32; var nom:str19);
var i : SInt32;
    err : OSErr;
begin
  ...
  err := ReadBufferDansFichierTexte(BaseSolitairesFic,MakeMemoryPointer(buffer),count);
  for i := 1 to 19 do
    begin
...
procedure LitTournoiNro(n : SInt32; var nom:str29);
var i : SInt32;
    c : char;
    err : OSErr;
begin
...
begin
  ...
  if err <> 0 then
    begin
      ...
      BeginDialog;
      dp := MyGetNewDialog(PasTrouveThordbaID,FenetreFictiveAvantPlan());
      if dp <> NIL then
        begin
          ParamText(fileName,'','','');
          err := SetDialogTracksCursor(dp,true);
        ...
        if (itemHit=PasTrouveAnnulerBouton)
          then
            ...
            begin
              ...
              if reply.good
                then
                  if (Pos('olitaires Cassio',reply.fname)>0) |
                     (Pos('olitaire Cassio' ,reply.fname)>0)
                   then
                    begin
                      CheminAccesSolitaireCassio^^ := reply.fname;
                      SolitairesRefVol := reply.vrefNum;
                      err := SetVol(NIL,SolitairesRefVol);
                      {WriteStringAndNumAt('Solitaires RefNum :',SolitairesRefVol,10,40);
                      WriteStringAndNumAt('Cassio RefNum :',volumeRefCassio,10,60);
                      AttendFrappeClavier;
                      }
                      err := FichierTexteExiste(reply.fname,0,FichierReferencesSolitaires);
...
function NroSolitaireAuHazard(var numeroDeCoup : SInt32; var couleur : SInt32; var commentaire : str255) : SInt32;
...
begin
  ...
  {WriteStringAndNumDansRapport('longFichierReferencesSolitaires=',longFichierReferencesSolitaires);}
  ...
  {WriteStringAndNumDansRapport('   PositionDansFichier=',PositionDansFichier);}
  ...
  {WritelnStringAndNumDansRapport('  uneLigne='+uneligne+'  , length=',Length(uneligne));}
  ...
  if aux>0 
   then 
    begin
      ...
      StringToNum(s,numeroDeCoup);
      if uneligne[aux+6]<>'0'
       then GetIndString(commentaire,TextesSolitairesID,1)
       else GetIndString(commentaire,TextesSolitairesID,3);
       ...
       if aux>0 
         then 
	         begin
	           ...
	           if uneligne[aux+6]<>'0'
	             then GetIndString(commentaire,TextesSolitairesID,2)
	             else GetIndString(commentaire,TextesSolitairesID,4);
	           commentaire := Concat(commentaire,'   ');
	         end
         else
           begin
  ...
  nroPartie := 0;
  StringToNum(s,nroPartie);
  NroSolitaireAuHazard := nroPartie;
end;

procedure FabriqueCommentaire(nroPart : SInt32; var comment : str255);
var nom,s,s2 : str255;
    s19:str19;
    s29:str29;
    i,long : SInt32;
...
begin
  ...
  s19 := DeleteSpacesBefore(s19,Length(s19));
  {while s19[Length(s19)]=' ' do Delete(s19,Length(s19),1);}
  ...
  {if referencesCompletes then 
   begin
     NumToString(nroPart,s);
     comment := Concat(comment,'  (N° ',s,StringOf(')'));
   end;}
  
  {
  NumToString(Partiebuff^.scoreEtTheorik[0],s2);
  ParametresOuvrirThor^^[5] := s2;
  }
end;

procedure LitUnSolitaireAuHazard(var PositionEtCommentaire : str255);
  ...
  if RejoueJusquAuCoup(numeroDeCoup-1) then
    begin
      ...
      if referencesCompletes & not(EnVieille3D()) then
        begin
          NumToString(numerodecoup,s);
          commentaire := commentaire+', c.'+s;
        end;
      CompilerPosition(platSol,s);
      PositionEtCommentaire := s+commentaire;
      NumToString(nroDeCeSolitaire,s);
      if Length(s)<5 then for i := 1 to (5-Length(s)) do s := Concat(s,StringOf(' '));
                   ...
                   if (itemHit=PasTrouveAnnulerBouton)
                     then
                       ...
                       begin
                         ...
                         if reply.good 
                           then
                             if (Pos('litaires.dba',reply.fname)>0) | (Pos('LITAIRES.dba',reply.fname)>0) | 
                                (Pos('LITAIRES.DBA',reply.fname)>0) | (Pos('LITAIRE.DBA',reply.fname)>0) | 
                                (Pos('litaire.dba',reply.fname)>0)  | (Pos('LITAIRE.dba',reply.fname)>0) then
                               begin
             ...
             then
               begin
                 ...
                 for index := 1 to SolitairesEnMemoire do
                  ...
                  {  WriteStringAndNumAt(posEtComment+'  ',index,2,11*index);  }
                   end;
...
begin
  ...
  theDialog := MyGetNewDialog(kDialogueConfigurationSolitairesID, FenetreFictiveAvantPlan());
  if theDialog <> NIL then
    begin
...
var dp : DialogPtr;
    ...
    FiltreSolitaireUPP : ModalFilterUPP;
      ...
      if prof <= 30 then
        begin
          ...
          if EstUnSolitaire(seulCoup,seuleDef,couleur,prof,nbblanc,nbnoir,jeuCourant,
            ...
            then 
              begin
                dp := MyGetNewDialog(SolitaireStrictID,FenetreFictiveAvantPlan());
                FiltreSolitaireUPP := NewModalFilterUPP(@MyFiltreClassique);
                    ...
                    then
                      begin
                        if EstUnSolitaire(seulCoup,seuleDef,couleur,prof,nbblanc,nbnoir,jeuCourant,
                          ...
                          then 
                            begin
                              dp := MyGetNewDialog(SolitaireLargeID,FenetreFictiveAvantPlan());
                              FiltreSolitaireUPP := NewModalFilterUPP(@MyFiltreClassique);
                              propositiondeValider := true;
                            end
                          else 
                            begin
                              if (interruptionReflexion = pasdinterruption) then 
                                begin
                                  dp := MyGetNewDialog(PasSolitaireID,FenetreFictiveAvantPlan());
                                  FiltreSolitaireUPP := NIL
                                end;
                              case causerejet of
                                1  : GetIndString(s,TextesSolitairesID,21);
                                2  : GetIndString(s,TextesSolitairesID,22);
                                3  : GetIndString(s,TextesSolitairesID,23);
                                4  : GetIndString(s,TextesSolitairesID,24);
                                5  : GetIndString(s,TextesSolitairesID,25);
                                6  : GetIndString(s,TextesSolitairesID,26);
                                7  : GetIndString(s,TextesSolitairesID,27);
                                8  : GetIndString(s,TextesSolitairesID,28);                
                              end;
                              case couleur of
                                pionNoir  : GetIndString(s1,TextesSolitairesID,19);
                                pionBlanc : GetIndString(s1,TextesSolitairesID,20);
                              end;{case} 
                              s := ParamStr(s,s1,'','','');
                            end;
                      end
          ...
          if dp <> NIL then
            begin
              if s<>'' then ParamText(s,'','','');
              err := SetDialogTracksCursor(dp,true);
              repeat
                ModalDialog(FiltreSolitaireUPP,itemHit);
              until (itemHit=OK) | (itemHit=Annuler) | (itemHit=Valider);
              MyDisposeModalFilterUPP(FiltreSolitaireUPP);
              MyDisposeDialog(dp);
              if HasGotEvent(updateMask,theEvent,0,NIL) then TraiteOneEvenement;
              if (itemHit=Valider) & propositiondeValider then
                begin
                  if couleurMacintosh=couleur then 
                    begin
                      couleurMacintosh := -couleurMacintosh;
                      reponsePrete := false;
                      RefleSurTempsJoueur := false;
                      LanceInterruption(interruptionSimple,'DoEstUnSolitaire (1)');
                      vaDepasserTemps := false;
                    end;
                  SensLargeSolitaire := true;
                  if HumCtreHum then 
                    begin
                      ...
                      DessineCourbe(0,nbreCoup,kCourbeColoree,'DoEstUnSolitaire');
                  ...
                  if score>0 
                    then if couleur = pionNoir
                           then GetIndString(s,TextesSolitairesID,1)  {Noir joue et gagne…}
                           else GetIndString(s,TextesSolitairesID,2)  {Blanc joue et gagne…}
                    else if couleur = pionNoir
                           then GetIndString(s,TextesSolitairesID,3)  {Noir joue et annule…}
                           else GetIndString(s,TextesSolitairesID,4); {Noir joue et annule…}
    ...
    if CassioEstEnModeSolitaire() then
      begin
        ...
        if Pos(s,commentaire)>0 
          then dp := MyGetNewDialog(FelicitationGainID ,FenetreFictiveAvantPlan())
          else dp := MyGetNewDialog(FelicitationNulleID,FenetreFictiveAvantPlan());
        if dp <> NIL then
          begin
            ...
            if avecSon then 
              begin
                ...
                PlaySoundAsynchrone(SonFelicitationsID,theChannel);
                SetSoundVolumeOfChannel(theChannel,100);
              end;
  ...
  if peutfeliciter & not(HumCtreHum)
    then 
      begin
        ...
        commentaire := CommentaireSolitaire^^;
        GetIndString(s,TextesSolitairesID,19);
        if Pos(s,commentaire)>0 
