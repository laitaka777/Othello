UNIT UnitTestGrapheApprentissage;
...
USES UnitRapportImplementation,UnitOth1,UnitRapport,SNStrings,UnitFenetres,MyMathUtils,
     UnitEntreesSortiesGraphe;
...
begin
  if not(GrapheApprentissageExiste(nomGrapheApprentissage,fic,grapheDejaOuvertALArrivee))
    ...
    else if FermeGrapheApprentissage(fic) then;
    ...
    begin
     ...
     {VideGrapheApprentissage(fic);
      ...
      if not(grapheDejaOuvertALArrivee) then
        if FermeGrapheApprentissage(fic) then;
    end;
end;
