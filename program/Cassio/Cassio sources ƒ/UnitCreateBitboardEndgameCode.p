UNIT UnitCreateBitboardEndgameCode;
...
USES UnitScannerOthellistique,UnitStrategie,UnitFichiersTEXT, UnitRapport, 
     UnitBitboardModifPlat,SNStrings,UnitMacExtras;
...
function BitboardName(color : SInt16; isLow : boolean) : string;
begin
  case color of
    _MY_BITS_ :
      if isLow 
...
function StringToJanSquare(s : string) : SInt16; 
var col,line : SInt16; 
begin
  col  := ord(s[1]) - ord('a');
  line := ord(s[2]) - ord('1');
...
function JanSquareToString(square : SInt16) : string;
var col,line : SInt16; 
begin
  line := square div 8;
  col  := square mod 8;
...
procedure WritelnDansFichierEndgame(s : string);
var err : OSErr;
begin {$UNUSED err}
...
procedure WriteDansFichierEndgame(s : string);
var err : OSErr;
begin {$UNUSED err}
...
procedure WritelnSansMargeDansFichierEndgame(s : string);
var err : OSErr;
begin {$UNUSED err}
  err := WritelnDansFichierTexte(fichierCodeGenere,s);
end;

procedure WriteSansMargeDansFichierEndgame(s : string);
var err : OSErr;
begin {$UNUSED err}
  err := WriteDansFichierTexte(fichierCodeGenere,s);
end;

procedure WritelnCommentaireDansFichierEndgame(s : string);
var err : OSErr;
begin {$UNUSED err}
...
procedure WriteCommentaireDansFichierEndgame(s : string);
var err : OSErr;
begin {$UNUSED err}
  (* WriteDansRapport(' { '+s+' }'); *)
  err := WriteDansFichierTexte(fichierCodeGenere,' { '+s+' }'); 
...
function CreateFunctionLine(whichName : string;whichType : SInt16) : string;
var s : string;
begin
  s := '';
  if generatingPascal
...
function CreateFunctionCall(whichName : string) : string;
var s : string;
begin
  s := '';
  if generatingPascal
...
procedure WriteFunctionLine(whichName : string;whichType : SInt16);
var s : string;
begin
...
procedure WriteFunctionCall(whichName : string);
var s : string;
begin
...
procedure WriteAffectation(variableName,rightSide : string);
begin
  if generatingPascal
...
procedure WriteReturns(whichName,result : string);
begin
  if generatingPascal
...
function ArrayReference(s : string) : string;
begin
  ArrayReference := Concat('[',s,']');
end;

function IntegerReference(num : SInt32) : string;
begin
  IntegerReference := ArrayReference(NumEnString(num));
end;

function CreatePlusEgal(variable,increment : string) : string;
begin
  if generatingPascal
...
function CreateMoinsEgal(variable,decrement : string) : string;
begin
  if generatingPascal
...
procedure WritePlusEgal(variable,increment : string);
begin
  WritelnDansFichierEndgame('  ' + CreatePlusEgal(variable,increment));
end;

procedure WriteMoinsEgal(variable,increment : string);
begin
...
procedure WriteIncrementation(variable : string);
begin
  if generatingPascal
...
procedure WriteDecrementation(variable : string);
begin
  if generatingPascal
...
procedure WriteIf(test : string;thenSurLaMemeLigne : boolean);
begin
  if generatingPascal
    then 
      begin
...
procedure WritelnIf(test : string;thenSurLaMemeLigne : boolean);
begin
...
procedure WritePlusEgalBitboard(variable,increment : string);
begin
...
begin
  ...
  WritelnStringAndNumDansRapport('BSL(16,1)=',BSL(16,1));
  WritelnStringAndNumDansRapport('BSR(16,1)=',BSR(16,1));
end;
...
begin
  ...
  WritelnStringAndNumDansRapport('CreateJansEndgameCode : err =',err);
  ...
  SetFileCreatorFichierTexte(fichierCodeGenere,'R*ch');
  SetFileTypeFichierTexte(fichierCodeGenere,'TEXT');
  
  WritelnStringAndNumDansRapport('CreateJansEndgameCode : terminé!   err=',err);  
end;
