...
USES UnitProperties,UnitPagesDePropertyList;
...
procedure WriteStringAndPropertyListDansRapport(s : str255;L : PropertyList);
procedure WritelnStringAndPropertyListDansRapport(s : str255;L : PropertyList); 
...
USES UnitServicesDialogs,UnitRapport,UnitScannerOthellistique;
...
begin
  ...
  { gestion speciale pour les proprietes dont l'argument est un ensemble de cases :
    s'il existe deja une propriete du meme type dans la liste, on ne rajoute pas 
    une nouvelle propriete de ce type, on fait plutot la difference des deux 
    ensembles; si cette difference est vide, on detruit la propriete}
  if (prop.stockage=StockageEnEnsembleDeCases) then  
    begin
      aux := SelectFirstPropertyOfTypes([prop.genre],L);
      if aux <> NIL then
