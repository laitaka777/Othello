...
USES UnitRapportImplementation,UnitServicesRapport,UnitRapport,UnitOth2,UnitNormalisation,
     MyTypes,UnitScannerOthellistique,MyMathUtils,UnitServicesMemoire,SNStrings;
...
var debut,fin,k : SInt32;
    ...
    c1,c2 : char;
    s,s1,s2,s3,s4,filename,ligne,pattern,remplacement,reste : str255;
    whichSquare : SInt16; 
  ...
  procedure RemplacerToutesOccurences(pattern,chaineRemplacement : str255;motsEntiers : boolean);
  var remplacer,trouve,refus : boolean;
      from,position,k,delta : SInt32;
      longueurPattern,longueurRemplacement : SInt32;
  begin
    longueurPattern := Length(pattern);
    longueurRemplacement := Length(chaineRemplacement);
...
begin
  ...
  if SelectionRapportNonVide() then
    begin
						    ...
						    begin
						      ...
						      if (erreurES=NoErr) & (ligne <> '') & (ligne[1] <> '%') then
		                        ...
		                        peutRemplacer^[k] := true;
						              ...
						              {WritelnStringAndNumDansRapport('numeroPasse = ',numeroPasse);}
						            end 
						          else
						            begin
      ...
      if (peutRemplacer <> NIL) then DisposePtr(Ptr(peutRemplacer));
    end;
