...
function AB_simple(var pl : plateauOthello; var joua : plBool; var bstBef : SInt32;coul,prof,alpha,beta,nBla,nNoi : SInt32; var fr : InfoFrontRec;canDoProbCut : boolean) : SInt32;
function AB_tore(var pl : plateauOthello; var bstBef : SInt32;coul,prof,alpha,beta,nBla,nNoi : SInt32) : SInt32;

function ProofNumberMapping(v,valeurCible : SInt32;facteurExponentiel : extended) : extended;
function ProofNumber(var plat : plateauOthello;depth,trait,couleurProof,nbCasesVides : SInt32;alpha_PN,beta_PN : extended; var casesVides:listeVides) : extended;
function ProofNumberMilieu(var pl : plateauOthello;depth,trait,couleurProof,valCible,nbCasesVides : SInt32;alpha_PN,beta_PN : extended; var casesVides:listeVides; var infosMilieu:InfosMilieuRec) : extended;


var nbEvaluationsPourProofNumber : SInt32;
    exponentialMappingProofNumber : extended;
    quantumProofNumber : extended;
...
USES UnitEvaluation,UnitUtilitaires,UnitStrategie,UnitTore,UnitOth2,UnitGestionDuTemps;
...
function AB_simple(var pl : plateauOthello; var joua : plBool; var bstBef : SInt32;
                  coul,prof,alpha,beta,nBla,nNoi : SInt32; var fr : InfoFrontRec;canDoProbCut : boolean) : SInt32;
var platEssai : plateauOthello;
  ...
  begin
    ...
    {if effetspecial & (nroEffetspecial >= 4) 
      then profMinimalePourTrierAvecEval := 2
      else} profMinimalePourTrierAvecEval := 3;
...
begin
 ...
 begin
   ...
   {
   SetRect(lignerect,50,225-prof*36,100,250-prof*36);
   EraseRect(lignerect);   
   NumToString(alpha,s1);
   NumToString(beta,s2);
   Moveto(50,238-prof*36);
   DrawString(s1);
   Moveto(50,250-prof*36);
   DrawString(s2);
   }
   ...
   EssaieSetPortWindowPlateau;
	 WriteStringAndNumAt('prof=',prof,10,10);
	 Ecritpositionat(platEssai,10,20);
	 WriteStringAndNumAt('max=',maxCourant,10,140);
	 WriteStringAndNumAt('coul=',coul,10,150);     
   WriteStringAndNumAt('nbBlcEssai=',nbBlcEssai,100,150);     
   WriteStringAndNumAt('nbNrEssai=',nbNrEssai,200,150);       
	 SysBeep(0);
	 AttendFrappeClavier; 
   }
        ...
        if DoitPasser(adversaire,pl,joua) then
          begin
            ...
            if utilisationNouvelleEval
              then AB_simple := 100*note
              else AB_simple := 500*note;
          end          
        else
  ...
  if depth <= 0
    then
      begin
        ...
        WritelnStringAndNumDansRapport('couleurProof = ',couleurProof);
        WritelnStringAndNumDansRapport('eval = ',eval);
        WritelnStringAndNumDansRapport('valCible = ',valCible);
      ...
      begin
        if DoitPasserTore(adversaire,pl) then
          begin
            ...
            if utilisationNouvelleEval
              then AB_tore := 100*note
              else AB_tore := 500*note;
          end          
        else
