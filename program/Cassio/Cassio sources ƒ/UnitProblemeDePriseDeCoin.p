...
procedure SetIntervaleDeDifficultePourProblemeDePriseDeCoin(difficulteMin,difficulteMax : SInt32);
procedure GetIntervaleDeDifficultePourProblemeDePriseDeCoin(var difficulteMin,difficulteMax : SInt32);
...
function EstUnEnonceNumeroteDeProblemeDeCoin(const s : str255; var numero : SInt32) : boolean;
...
function GetNumeroDuProblemeDePriseDeCoinDansCetEnonce(const chaineEnonce : str255) : SInt32;
function GetEnonceProblemeDePriseDeCoinEnChaine(var enonce : ProblemePriseDeCoin) : str255;
function GetEnonceCourtProblemeDePriseDeCoinEnChaine(var enonce : ProblemePriseDeCoin) : str255;
function GetEnonceNumeroteProblemeDePriseDeCoin(var enonce : ProblemePriseDeCoin) : str255;
...
USES UnitStrategie,UnitListe,UnitOth1,SNStrings,UnitSetUp,UnitRapport,UnitActions,UnitGestionDuTemps,
     UnitRapportImplementation,UnitScannerOthellistique,UnitSolitaire,UnitJeu,UnitMacExtras;
...
var pilePositionsIntermediaires : array[0..kHorizonMaxProblemesPriseDeCoin] of PositionEtTraitRec;
    ...
    gProblemePrecedent : record
                           nbProblemesDejaProposes : longint;
...
procedure InitUnitProblemeDePriseDeCoin;
begin
  SetIntervaleDeDifficultePourProblemeDePriseDeCoin(2,5);
                  ...
                  begin
                    ...
                    (*if (nbCoupsJouesParAttaquant = 2) & (pileDesCoupsJouees[0] = 85) then
                      begin
                        ...
                        WritelnStringAndNumDansRapport('valeur = ',valeurDuCoup);
                      end;*)
                    ...
                    if (nbCoupsJouesParAttaquant = 0) & 
                      ...
                      then 
                        begin
                          ...
                          WritelnStringAndNumDansRapport('longueur = ',valeurDuCoup);}
...
function EstUnEnonceNumeroteDeProblemeDeCoin(const s : str255; var numero : SInt32) : boolean;
var subStringSignificative : str255;
begin
      ...
      kRejetADejaUnCoin :
        if GetTraitOfPosition(enonce.currentBoard) = pionNoir
          then {Noir a déja un coin}
               WritelnDansRapport(ParamStr(ReadStringFromRessource(kTextesProblemesDeCoin,12),'','','',''))
          else {Blanc a déja un coin}
               WritelnDansRapport(ParamStr(ReadStringFromRessource(kTextesProblemesDeCoin,13),'','','',''));
               
      kRejetPeutDejaPrendreUnCoin :
        if GetTraitOfPosition(enonce.currentBoard) = pionNoir
          then {Noir peut déja prendre un coin}
               WritelnDansRapport(ParamStr(ReadStringFromRessource(kTextesProblemesDeCoin,6),'','','',''))
          else {Blanc peut déja prendre un coin}
               WritelnDansRapport(ParamStr(ReadStringFromRessource(kTextesProblemesDeCoin,7),'','','',''));
...
procedure ChercherProblemeDePriseDeCoinCettePosition(var plateau : plateauOthello; var jouable : plBool; var frontiere : InfoFrontRec;
...
begin
  if (nbPartiesActives > 0) & windowListeOpen &
    ...
    begin
      ...
      GetIntervaleDeDifficultePourProblemeDePriseDeCoin(tempDifficulteMin,tempDifficulteMax);
      {SetIntervaleDeDifficultePourProblemeDePriseDeCoin(2,5);}
      ...
      if gRechercheProblemesDePriseDeCoin.interrompue then
        begin
          ...
          {WritelnStringAndNumDansRapport('temps = ',TickCount() - tickDepartRechercheDesProblemes);}
      ...
      SetIntervaleDeDifficultePourProblemeDePriseDeCoin(tempDifficulteMin,tempDifficulteMax);
    end;
end;
  ...
  if not(gRechercheProblemesDePriseDeCoin.enCours) &
    ...
    begin
      ...
      repeat
        ...
        typeDeProbleme := Abs(Random()) mod 4;
        
        
        case typeDeProbleme of
          0 :
            begin
              lesCasesInterdites := [11,18,81,88,12,21,17,28,71{,82,78,87}];  {les cases C, et toutes les cases X sauf B7}
              premierCoup := 25;
              dernierCoup := 43;
            end;
          1 :
          ...
          2 : 
            begin
              lesCasesInterdites := [11,18,81,88,22,27,72,77]; {les cases X}
              premierCoup := 25;
              dernierCoup := 40;
            end;
          3 :
        ...
        SetIntervaleDeDifficultePourProblemeDePriseDeCoin(5,5);
        
        repeat
          PartagerLeTempsMachineAvecLesAutresProcess(kCassioGetsAll);
          
          GenereOuvertureAleatoireEquilibree(dernierCoup,-20000,20000,lesCasesInterdites,partie60);
      ...
      {WritelnStringAndNumDansRapport('Il a fallu '+NumEnString(compteur)+' parties aléatoires pour générer ce problème de type ',typeDeProbleme);}
