...
USES MacTypes, UnitFichiersTEXT, UnitGeneralSort;


TYPE

  TypeReel=real;
  TypeReelArray = array[0..0] of TypeReel;
...
USES fp,UnitServicesMemoire,UnitRapport;
        ...
        if (p2^[j]>eps) | (p2^[j] < -eps)
          then 
            begin
              ...
              if (aux>1.0) | (aux<=0.0) then aux := 1.0;
...
begin
  if (index<1) | (index>InfosTri.TailleVecteur)
    then
      begin
        WritelnStringAndNumDansRapport('pb dans LitIndexTriPointMultidimensionnel !! index=',index);
...
begin
  if (index<1) | (index>InfosTri.TailleVecteur)
    then
      begin
        WritelnStringAndNumDansRapport('pb dans AffecteIndexTriPointMultidimensionnel !! index=',index);
...
begin
  if (element1<1) | (element1>InfosTri.TailleVecteur) then
    begin
      WritelnStringAndNumDansRapport('pb dans OrdreTriPointMultidimensionnel !! element1=',element1);
      OrdreTriPointMultidimensionnel := false;
      exit(OrdreTriPointMultidimensionnel);
    end;
  if (element2<1) | (element2>InfosTri.TailleVecteur) then
    begin
      WritelnStringAndNumDansRapport('pb dans OrdreTriPointMultidimensionnel !! element2=',element2);
      OrdreTriPointMultidimensionnel := false;
      exit(OrdreTriPointMultidimensionnel);
    end;
