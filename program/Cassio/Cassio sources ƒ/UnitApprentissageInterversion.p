UNIT UnitApprentissageInterversion;
...
USES  UnitDefinitionsApprentissage,UnitAccesStructuresGraphe,UnitEntreesSortiesGraphe,
      UnitCalculsApprentissage,UnitPositionEtTraitSet;
...
procedure AjouteInterversionDansGraphe(var fichier : Graphe;ligne1,ligne2 : str255; var estNouvelle : boolean);
procedure EnleveFausseInterversion(var fichier : Graphe;ligne1,ligne2 : str255);
...
function NbInterversionsDeCettePartieDansGraphe(var fichier : Graphe; const whichGame : str255;dansRapport : boolean;tableLignes : TableParties60Ptr) : SInt32;
function InterversionDansLeGrapheApprentissage(const whichGame : str255;listerInterDansRapport : boolean;tableLignes : TableParties60Ptr) : boolean;
function NbFaconsArriverACetteCelluleDansGraphe(var fichier : Graphe;numCellule : SInt32;ecritVariantes : boolean; const s : str255) : SInt32;
...
USES UnitNormalisation,UnitInterversions,UnitJeu,UnitGameTree,UnitScannerOthellistique,UnitRapport,SNStrings,UnitPackedThorGame;
...
begin
  ...
  if Length(ligne1)=Length(ligne2) then
    begin
...
var nbInterversions,t : SInt16; 
    ...
    ligne1,ligne2:str120;
    ligne1_255,ligne2_255 : str255;
    changed : boolean;
begin
  ...
  if PositionEstDansLeGraphe(fichier,partie60,path) then
    begin
      ...
      if (orbiteInterversions.cardinal>=2) then
        begin
          if dansRapport then WritelnStringAndNumDansRapport('orbiteInterversions.cardinal = ',orbiteInterversions.cardinal);
      ...
      if nbChemins>1 then WritelnStringAndNumDansRapport('nbChemins=',nbChemins);
      }
...
function NbFaconsArriverACetteCelluleDansGraphe(var fichier : Graphe;numCellule : SInt32;ecritVariantes : boolean; const partie : str255) : SInt32;
  ...
  if not(HasPere(cellule))  { racine ? }
    then 
      begin
        ...
        if ecritVariantes then WritelnDansRapport(CoupEnStringEnMajuscules(GetCoup(cellule))+partie);
      end
    else
      begin
        LitOrbite(fichier,numCellule,orbite);
        ...
        for i := 1 to orbite.cardinal do
          begin
            ...
            if HasPere(cellule) then
              if ecritVariantes
                then somme := somme+NbFaconsArriverACetteCelluleDansGraphe(fichier,GetPere(cellule),true,CoupEnStringEnMajuscules(GetCoup(cellule))+partie)
