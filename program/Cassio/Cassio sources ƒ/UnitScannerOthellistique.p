...
USES MacTypes,StringTypes,UnitDefinitionsPackedThorGame,UnitOth0;
...
function CoupEnStringEnMajuscules(coup : SInt16) : string;
function CoupEnStringEnMinuscules(coup : SInt16) : string; 
function CoupEnString(coup : SInt16; enMajuscules : boolean) : string;
function StringEnCoup(const s : string) : SInt16; 
function ScannerStringPourTrouverCoup(debutScan : SInt16; const s : string; var positionDuCoupDansChaine : SInt16) : SInt16; 
function PositionDansStringAlphaEnCoup(const s : string;positionDansChaine : SInt16) : SInt16; 


{ Fonctions de traductions }
procedure TraductionThorEnAlphanumerique(const s : PackedThorGame; var result : str255);
procedure TraductionAlphanumeriqueEnThor(const s : str255; var result : PackedThorGame);
procedure TraductionThorEnSuedois(const s : PackedThorGame; var result : str60);
procedure CompacterPartieAlphanumerique(var s : str255;modification : SInt16);
...
function CalculeLeTraitApresTelCoup(n : SInt16; s : str255; platDebut : plateauOthello; traitDebut : SInt16) : SInt16; 


{ Fonctions de reconnaissance d'une partie d'othello dans une chaine }
function EstUnePartieOthello(var s : str255;compacterPartie : boolean) : boolean;
function EstUnePartieOthelloAvecMiroir(var s : str255) : boolean;
function EstUnePartieOthelloTerminee(var s : str255;compacterPartie : boolean; var nbPionsFinalNoirs,nbPionsFinalBlancs : SInt32) : boolean; 


{ Fonction cherchant simultanement une partie et des joueurs dans une chaine }
function TrouverPartieEtJoueursDansChaine(chaine : str255; var partieEnAlpha : str255; var numeroJoueur1,numeroJoueur2 : SInt32; var qualiteSolution : extended) : boolean;
...
USES UnitOth0,MyStrings,UnitRapport,MyMathUtils,UnitNormalisation,SNStrings,
     UnitImportDesNoms,UnitDefinitionsNouveauFormat,UnitStrategie,UnitPackedThorGame;
...
begin
  ...
  if (n >= (Length(s) div 2)) then n := Length(s) div 2;
  if (n >= 64) then n := 64;
  ok := true;
  for i := 1 to n do
    if ok then 
...
begin
  ...
  if partieToujoursLegale then
    repeat
      ...
      RecoitCaractere(s[k]);
      {
      WritelnStringAndNumDansRapport('k=',k);
      ...
      WritelnStringAndNumDansRapport('coup=',coup);
      }
      if coupComplet then
        begin
		      if (coup >= 11) & (coup <= 88) 
...
begin
  ...
  SymetriserPartieFormatAlphanumerique(chaineTest,axeVertical,1,length(chaineTest) div 2);
  if EstUnePartieOthello(chaineTest,true) then
    begin
      s := chaineTest;
      EstUnePartieOthelloAvecMiroir := true;
...
var s,lesJoueurs : str255;
  ...
  procedure RechercherCoupsCommencantPar(firstMove : str255;axeSymetrie : SInt32);
  var deb,fin,longueurDeCetteSolution : SInt32;
      reste,moves,lastMove : str255;
  begin
      ...
      begin
        ...
        if (deb > 0) then
          begin
            ...
            reste := RightOfString(s,Length(s)-deb+1);
            
            moves := reste;
            if EstUnePartieOthelloAvecMiroir(moves) then
              begin
                
                longueurDeCetteSolution := Length(moves) div 2;
...
begin  { TrouverPartieEtJoueursDansChaine }
  ...
  lesJoueurs := LeftOfString(chaine,indexDebutDesCoups-1) + RightOfString(chaine,Length(chaine)-indexFinDesCoups);
