...
static int hash_mask_zebra;
static int rehash_count;
static unsigned int hash_trans1 = 0;
static unsigned int hash_trans2 = 0;
static CompactHashEntry *hash_table_zebra;


CompactHashEntry wide_to_compact( HashEntry entry );
HashEntry compact_to_wide( CompactHashEntry compact_entry );
...
init_hash( int in_hash_bits ) {
  ...
  hash_mask_zebra = hash_size - 1;
  hash_table_zebra =
    (CompactHashEntry *) malloc( hash_size * sizeof( CompactHashEntry ) );
  rehash_count = 0;
}
...
resize_hash( int new_hash_bits ) {
  free( hash_table_zebra );
  init_hash( new_hash_bits );
  setup_hash( VALUE_TRUE );
}
...
setup_hash( int clear ) {
  ...
  if ( clear )
    for ( i = 0; i < hash_size; i++ ) {
      hash_table_zebra[i].key1_selectivity_flags_draft &= ~DRAFT_MASK;
      hash_table_zebra[i].key2 = 0;
    }
...
clear_hash_drafts( void ) {
  ...
  for ( i = 0; i < hash_size; i++ )  /* Set the draft to 0 */
    hash_table_zebra[i].key1_selectivity_flags_draft &= ~0x0FF;
}
...
add_hash( int reverse_mode,
  ...
  index1 = code1 & hash_mask_zebra;
  index2 = SECONDARY_HASH( index1 );
  if ( hash_table_zebra[index1].key2 == code2 )
    index = index1;
  else {
    if ( hash_table_zebra[index2].key2 == code2 )
      index = index2;
    else {
      if ( (hash_table_zebra[index1].key1_selectivity_flags_draft & DRAFT_MASK) <=
	   (hash_table_zebra[index2].key1_selectivity_flags_draft & DRAFT_MASK) )
	index = index1;
      else
	index = index2;
    }
  ...
  old_draft = hash_table_zebra[index].key1_selectivity_flags_draft & DRAFT_MASK;

  if ( flags & EXACT_VALUE )  /* Exact scores are potentially more useful */
    change_encouragment = 2;
  else
    change_encouragment = 0;
  if ( hash_table_zebra[index].key2 == code2 ) {
  ...
  hash_table_zebra[index] = wide_to_compact( entry );
}
...
find_hash( int reverse_mode ) {
  ...
  index2 = SECONDARY_HASH( index1 );
  if ( hash_table_zebra[index1].key2 == code2 ) {
    if ( (hash_table_zebra[index1].key1_selectivity_flags_draft & KEY1_MASK) ==
	 (code1 & KEY1_MASK) )
      return compact_to_wide( hash_table_zebra[index1] );
  }
  else if ( (hash_table_zebra[index2].key2 == code2 ) &&
	    ((hash_table_zebra[index2].key1_selectivity_flags_draft & KEY1_MASK) ==
	     (code1 & KEY1_MASK)) )
    return compact_to_wide( hash_table_zebra[index2] );
