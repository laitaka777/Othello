...
count_edge_stable( int color,
  ...
  edge_a1h1 = 3280 * EMPTY;
  if ( color == BLACKSQ ) {
    edge_a1h1 += (BLACKSQ - EMPTY) * base_conversion[col_bits.low & 255];
    edge_a1h1 += (WHITESQ - EMPTY) * base_conversion[opp_bits.low & 255];
  }
  else {
    edge_a1h1 += (WHITESQ - EMPTY) * base_conversion[col_bits.low & 255];
    edge_a1h1 += (BLACKSQ - EMPTY) * base_conversion[opp_bits.low & 255];
  }

 edge_a8h8 = 3280 * EMPTY;
  if ( color == BLACKSQ ) {
    edge_a8h8 += (BLACKSQ - EMPTY) * base_conversion[col_bits.high >> 24];
    edge_a8h8 += (WHITESQ - EMPTY) * base_conversion[opp_bits.high >> 24];
  }
  else {
    edge_a8h8 += (WHITESQ - EMPTY) * base_conversion[col_bits.high >> 24];
    edge_a8h8 += (BLACKSQ - EMPTY) * base_conversion[opp_bits.high >> 24];
  }

  edge_a1a8 = 3280 * EMPTY;
  ...
  if ( color == BLACKSQ ) {
    edge_a1a8 += (BLACKSQ - EMPTY) * base_conversion[col_mask];
    edge_a1a8 += (WHITESQ - EMPTY) * base_conversion[opp_mask];
  }
  else {
    edge_a1a8 += (WHITESQ - EMPTY) * base_conversion[col_mask];
    edge_a1a8 += (BLACKSQ - EMPTY) * base_conversion[opp_mask];
  }

  edge_h1h8 = 3280 * EMPTY;
  ...
  if ( color == BLACKSQ ) {
    edge_h1h8 += (BLACKSQ - EMPTY) * base_conversion[col_mask];
    edge_h1h8 += (WHITESQ - EMPTY) * base_conversion[opp_mask];
  }
  else {
    edge_h1h8 += (WHITESQ - EMPTY) * base_conversion[col_mask];
    edge_h1h8 += (BLACKSQ - EMPTY) * base_conversion[opp_mask];
  }
...
stability_search( BitBoard my_bits,
  ...
  for ( old_sq = END_MOVE_LIST_HEAD, sq = stab_move_list[old_sq].succ;
    ...
    if ( TestFlips_bitboard[sq]( my_bits, opp_bits ) ) {
      ...
      if ( max_depth > 1 ) {
	FULL_XOR( new_opp_bits, opp_bits, bb_flips );
	FULL_XOR( new_my_bits, my_bits, bb_flips );
	APPLY_XOR( new_my_bits, square_mask[sq] );
	stab_move_list[old_sq].succ = stab_move_list[sq].succ;
	stability_search( new_opp_bits, new_my_bits, OPPONENT( side_to_move ),
			  candidate_bits, max_depth - 1, VALUE_FALSE,
			  stability_nodes );
  ...
  if ( (mobility == 0) && !last_was_pass )
    stability_search( opp_bits, my_bits, OPPONENT( side_to_move ),
		      candidate_bits, max_depth, VALUE_TRUE, stability_nodes );
}
...
complete_stability_search( int *board,
  ...
  stability_search( my_bits, opp_bits, side_to_move, &candidate_bits,
		    MIN( empties, shallow_depth ), VALUE_FALSE, &stability_nodes );
  ...
  for ( i = 1; (i <= 8) && !abort; i++ )
	...
	stability_search( my_bits, opp_bits, side_to_move, &test_bits,
			  empties, VALUE_FALSE, &stability_nodes );
