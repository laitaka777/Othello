...
USES UnitMenus,UnitOth1,UnitOth2,UnitAffichageReflexion,UnitGestionDuTemps,UnitJeu,
     UnitRapport,UnitCarbonisation;
...
begin
  case genre of
    ...
    otherwise 
      begin
        WritelnStringAndNumDansRapport('WARNING !! genre inconnu dans GenreReflexionToPhasePartie, prévenez Stéphane ! genre = ',genre);
      end;
  end;
end;
...
begin
  ...
  if jeuInstantane & not(demo)
   then 
    ...
    if (GetCadence() = minutes3) & not(demo) then
      begin
...
begin
  ...
  AjusteSleep;
  if HumCtreHum & (aQuiDeJouer=couleurMacintosh) 
    then 
      begin
  ...
  LanceInterruption(interruptionSimple,'DoChangeHumCtreHum');  
...
begin
  ...
  reponsePrete := false;
  RefleSurTempsJoueur := false;
  LanceInterruption(interruptionSimple,'InterruptionCarPhasePartieChange');
