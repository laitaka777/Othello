...
USES UnitMacExtras,GestaltEqu,TextServices,TSMTE,UnitOth0;



var gVersionJaponaiseDeCassio : boolean;
...
{remplacement de TSMEvent pour gerer les evenements en japonais}
{$IFC NOT(TARGET_API_MAC_CARBON_DANS_CASSIO) }
function IntlTSMEvent(var event : eventRecord) : boolean;
{$ENDC}
...
USES UnitOth1,UnitDialog,SNStrings;
  ...
  if gHasTSMTE
    then
      begin
        ...
        doIt := true;
        {$ELSEC}
        doIt := (InitTSMAwareApplication() = NoErr);
        {$ENDC}
...
var 
	oldFont : SInt16; 
	keyboardScript:ScriptCode;
	quickdrawPort : CGrafPtr;
begin	
	{ make sure we have a port and it's not the Window Manager port }
	if (qdThePort() <> NIL) & (FrontWindow() <> NIL) then
	begin
	  quickdrawPort := qdThePort();
		oldFont := GetPortTextFont(quickdrawPort);
		keyboardScript := GetScriptManagerVariable(smKeyScript);
		if (FontToScript(oldFont) <> keyboardScript) then
			TextFont(GetScriptVariable(keyboardScript, smScriptAppFond));
	end;
	IntlTSMEvent := TSMEvent(event);
end;
{$ENDC}
...
{$IFC NOT(TARGET_API_MAC_CARBON_DANS_CASSIO) }
  if gHasTextServices then err := CloseTSMAwareApplication();
{$ENDC}
