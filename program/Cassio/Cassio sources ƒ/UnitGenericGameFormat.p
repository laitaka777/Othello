...
USES UnitFichiersTEXT,UnitPositionEtTrait,UnitOthelloGeneralise,UnitZoneMemoire,UnitFormatsFichiers;
...
function StringSGFEnCoup(const s : str255; var square_x,square_y : SInt16) : boolean;

function ParserChainePositionInitialeNxN(var posInitiale : BigOthelloRec) : str255;
function ParserChaineCoupsPourDiagrammeNxN() : str255;
...
function GetChainePositionCouranteNxN(const chainePositionInitiale : str255) : str255;
function GetChaineCoupsEnAlphaNxN() : str255;
function GetNomDesNoirsNxN() : str255;
function GetNomDesBlancsNxN() : str255;
function GetNomTournoiNxN() : str255;


{ Gestion du presse-papier pour les diagrammes 10x10 }

procedure CopierEnMacDraw10x10(const chainePositionInitiale,chainePosition,chaineCoups : str255);
...
function ReadEnregistrementDansZoneMemoireSGF_ou_GGF(var theZone : ZoneMemoire; whichFormat : formats_connus; var result : PartieFormatGGFRec) : OSErr;
function GetPositionInitialeEtPartieDansFichierSGF_ou_GGF_8x8(var fic : FichierTEXT; whichFormat : formats_connus; var posInitiale : PositionEtTraitRec; var coups : str255) : OSErr;
...
USES MyStrings,UnitFormatsFichiers,UnitDiagramFforum,UnitNewGeneral,UnitOth1,
     UnitRapportImplementation,UnitRapport,UnitDialog,UnitServicesDialogs,MyFileSystemUtils,
     SNStrings,UnitFenetres;
...
var gLecture_NxN : 
      record
        ...
        zoneMemoireLecture                   : ZoneMemoire;
        ...
        chaineCoupsEnAlpha                   : str255;
        chainePositionDiagramme              : str255;
        nomDesNoirs                          : str120;
        nomDesBlancs                         : str120;
        nomTournoi                           : str120;
      end;
...
var err : OSErr;
begin
  err := SetPositionMarqueurZoneMemoire(gLecture_NxN.zoneMemoireLecture,gLecture_NxN.marqueurAuDebutDeLaLectureDeLaPartie);
...
begin
  with gLecture_NxN do
    begin
      ...
      marqueurAuDebutDeLaLectureDeLaPartie := GetPositionMarqueurZoneMemoire(zoneMemoireLecture);
      {WritelnStringAndNumDansRapport('Dans BeginParserPartieNxN, marqueurAuDebutDeLaLectureDeLaPartie = ',marqueurAuDebutDeLaLectureDeLaPartie);}
      ResetLecture_NxN;
     end;
end;
...
begin
  ...
  {WritelnStringAndNumDansRapport('Dans EndParserPartieNxN, marqueur = ',GetPositionMarqueurZoneMemoire(gLecture_NxN.zoneMemoireLecture));}
end;
...
begin
  err := GetNextCharOfZoneMemoire(gLecture_NxN.zoneMemoireLecture,c);
  while (err = NoErr) do
    begin
...
function UtilisateurChoisitFichier_NxN(N : SInt32; var theFic : FichierTEXT) : OSErr;
var reply : SFReply;
    ok : boolean;
    nomComplet : str255;
...
begin
  ...
  with gLecture_NxN do
    ...
    begin
      ...
      BeginDialog;
		  ok := GetFileName(reply,'TEXT','????','????','????',mySpec);
		  EndDialog;
		  
		  if ok then
		    begin
		      s1 := NumEnString(N) + 'x' + NumEnString(N); {'10x10'}
		      if (Pos(s1,reply.fName) <= 0) 
		        then
		          begin
		            AlerteSimple('Le nom du fichier doit contenir « '+s1+' »');
		          end
		        else
				      begin
				        nomComplet := GetFullPathOfFSSpec(mySpec);
                
                positionArobase := Pos('@',reply.fName);
                if (positionArobase <= 0)
                  then numeroDuCoupPourLeDiagramme := 10000
                  else 
                    begin
                      s := TPCopy(reply.fName,positionArobase+1,10);
  ...
  if longueur >= 2 then
    begin
      ...
      repeat
        ...
        { WritelnStringAndNumDansRapport('x = '+NumEnString(x) + '  y = ',y); }
...
function ScanChaineValeurProperty_NxN() : str255;
var s : str255;
    c : char;
    err : OSErr;
    longueur : SInt16; 
...
function ParserChainePositionInitialeNxN(var posInitiale : BigOthelloRec) : str255;
var chaineBlancs,chaineNoirs,s,s1 : str255;
    c : char;
    err : OSErr;
		        ...
		        if (GetPreviousCharFichier_NxN(-2) = 'B') & 
			         ...
			         (GetPreviousCharFichier_NxN(0) = '[') then
			          begin
			            ...
			            WritelnStringANdNumDansRapport('longueur = ',Length(s));}
			            s1 := NumEnString(N);   { "10" }
			            if (TPCopy(s,1,Length(s1)) = s1) then  
			              begin
			                s := TPCopy(s, Length(s1) + 1 , N*N + 1);
			                s := EnleveEspacesDeGauche(s);
			                {WritelnDansRapport('s = '+s);
			                writelnStringANdNumDansRapport('longueur = ',Length(s));}
			                if (Length(s) = (N*N + 1)) then
			                  begin
			                    for i := 1 to N do
			                    for j := 1 to N do
				                    begin
...
var s : str255;
    ...
    chaineCoupsPourDiagramme : str255;
  ...
  with gLecture_NxN do
    begin
      ...
      chaineCoupsPourDiagramme := '';
		  ...
		  {
		  WritelnStringAndNumDansRapport('numeroDuCoupPourLeDiagramme = ',numeroDuCoupPourLeDiagramme);
		  WritelnStringAndNumDansRapport('numeroDuCoupCourant = ',numeroDuCoupCourant);
		  WritelnStringAndNumDansRapport('formatFichier.format = ',SInt32(formatFichier.format));
		  }
...
begin
  ...
  SetTailleOthelloPourDiagrammeFForum(10,10);
...
procedure DoDiagramme10x10;
var chainePositionInitiale : str255;
    chaineCoups : str255;
...
begin
  ...
  if UtilisateurChoisitFichier_NxN(10,theFic) = NoErr then
    ...
    begin
      ...
      with gLecture_NxN do
	      if TypeDeFichierEstConnu(theFic,formatFichier,err) & 
	         ...
	         (formatFichier.tailleOthellier = 10) then
	        ...
	        begin
	          ...
	          zoneMemoireLecture := MakeZoneMemoireFichier(theFic.nomFichier,0);
	          lectureEnCours := ZoneMemoireEstCorrecte(zoneMemoireLecture);
	          
	          {
	          WritelnDansRapport('OK, zoneMemoireLecture(zone) dans DoDiagrammeNxN');
	          AttendFrappeClavier;
	          ...
	          if lectureEnCours then
	            begin
	              ...
	              BeginParserPartieNxN;
					      ...
					      if (chainePositionInitiale <> '') then
					        CopierEnMacDraw10x10(chainePositionInitiale,chainePosition,chaineCoups);
					        
					      SetTailleOthelloPourDiagrammeFForum(oldTaille.h,oldTaille.v);
					        
					      EndLecture_NxN;
					      DisposeZoneMemoire(gLecture_NxN.zoneMemoireLecture);
	            end;
...
function ReadEnregistrementDansZoneMemoireSGF_ou_GGF(var theZone : ZoneMemoire; whichFormat : formats_connus; var result : PartieFormatGGFRec) : OSErr;
var zoneMemoireOK : boolean;
    aux : str255; 
    N : SInt32;
begin

  ReadEnregistrementDansZoneMemoireSGF_ou_GGF := -1; 
  
  with gLecture_NxN do
    begin
    
      formatFichier.tailleOthellier  := 8;            {FIXME : ReadEnregistrementDansZoneMemoireSGF_ou_GGF ne marche que pour du 8x8}
      ...
      ecrireEnClairInfosDansRapport := false;
	    ...
	    zoneMemoireOK    := ZoneMemoireEstCorrecte(theZone);
	        
	    if zoneMemoireOK then
	      begin
	        lectureEnCours     := true;
	        
	        zoneMemoireLecture := theZone;
	        BeginParserPartieNxN;
          ...
          aux := ParserChainePositionInitialeNxN(result.positionInitiale);
			    ...
			    EndParserPartieNxN;
			    theZone := ZoneMemoireLecture;
	        
	        ReadEnregistrementDansZoneMemoireSGF_ou_GGF := NoErr;
	      end;
  ...
  if not(FichierTexteEstOuvert(fic)) then
	  if not(gLecture_NxN.lectureEnCours) then  
	    with gLecture_NxN do
          ...
          myZone         := MakeZoneMemoireFichier(fic.nomFichier,0);
	        lectureEnCours := ZoneMemoireEstCorrecte(myZone);
	        
	        if lectureEnCours then
	          begin
	            BeginLecture_NxN;
	            myError := ReadEnregistrementDansZoneMemoireSGF_ou_GGF(myZone,whichFormat,theGame);
	            
	            posInitiale := BigOthelloEnPositionEtTrait(theGame.positionInitiale);
	            coups       := theGame.coupsEnAlpha;
	            
	            EndLecture_NxN;
	            DisposeZoneMemoire(myZone);
	          end;
	    ...
	    with gLecture_NxN do
          ...
          myZone         := MakeZoneMemoireFichier(fic.nomFichier,0);
	        ...
	        if lectureEnCours then
	          begin
	            BeginLecture_NxN;
	            myError := ReadEnregistrementDansZoneMemoireSGF_ou_GGF(myZone,whichFormat,infos);
