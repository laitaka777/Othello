...
function MinimisationParBrentAvecDerivee(derivationNumerique : boolean;
                                         f:FonctionReelle;   {calcul de f}
                                         diff:DifferentielleReelle; {calcul simultanŽ de f et de f' : passer bidDiff si l'on veut utiliser la derivation numŽriqe}
...
USES UnitServicesDialogs,UnitRapport,SNStrings;
...
procedure DoNothing(x : TypeReel);
begin
  x := x;
end;
...
begin
  ...
  fret := MinimisationParBrentAvecDerivee(false,FonctionLigne,bidDiff,ax,xx,bx,tol,xmin);
...
begin
  ...
  for its := 1 to itmax do
    begin
      ...
      if 2*Abs(fret-fp) <= ftol*(Abs(fret)+Abs(fp)+eps) then goto finNormale;
      ...
      if gg = 0.0 then goto finNormale;  {(norme du) gradient = 0 ? gagne !}
      gam := dgg/gg;
      for j := 1 to n do
        begin
          g^[j] := -xi^[j];
  ...
  AlerteSimple('Pause dans la routine MinimisationMultidimensionnelleParConjugateGradient : trop d''iterations');
