...
USES UnitRapport,UnitFinaleFast,UnitSuperviseur,UnitPhasesPartie,UnitIconisation,MyMemory,UnitJeu,
     UnitMilieuDePartie,UnitAfficheArbreDeJeuCourant,UnitHashTableExacte,UnitActions,UnitListe,
     UnitServicesRapport,UnitRapportImplementation,UnitCarbonisation,UnitUtilitaires,UnitOth1,
     UnitArbreDeJeuCourant,UnitAffichageReflexion,UnitJaponais,UnitMenus,UnitGestionDuTemps,MyStrings,
     UnitDialog,UnitOth2,UnitScannerOthellistique,SNStrings,UnitCourbe,UnitFenetres,UnitCouleur,
     UnitNormalisation,UnitSetUp;
...
begin
  if seulementParametrage then
    begin
      ...
      SetControlTitle(ControlHandle(itemHandle),s);
...
begin
  ...
  BeginDialog;
	...
	dialogueAnalyseRetrograde := MyGetNewDialog(DialogueRetrogradeID, FenetreFictiveAvantPlan());
	if dialogueAnalyseRetrograde <> NIL then
	  begin
	    LoadMenusRetrograde;
  ...
  for j := 30 to 60 do
    with analyseRetrograde do
      begin
        s1 := NumEnString(i);
        s := NumEnString(j);
        WritelnStringAndNumDansRapport('demande['+s+','+s1+'].genre=',demande[j,i].genre);
        WritelnStringAndNumDansRapport('demande['+s+','+s1+'].tempsAlloueParCoup=',demande[j,i].tempsAlloueParCoup);
        {WritelnStringAndNumDansRapport('demande['+s+','+s1+'].profondeur=',demande[j,i].profondeur);}
      end;
end;
...
var s,passeStr : str255;
    oldScript : SInt32;
    oldInterruption : SInt16; 
begin
 GetCurrentScript(oldScript);
 case numeroPasse of
   1 : GetIndString(passeStr,TextesRetrogradeID,27);  {"première passe"}
   2 : GetIndString(passeStr,TextesRetrogradeID,28);  {"deuxième passe"}
   3 : GetIndString(passeStr,TextesRetrogradeID,29);  {"troisième passe"}
   4 : GetIndString(passeStr,TextesRetrogradeID,30);  {"quatrième passe"}
   5 : GetIndString(passeStr,TextesRetrogradeID,31);  {"cinquième passe"}
   otherwise
     begin
       GetIndString(passeStr,TextesRetrogradeID,32);  {"passe numéro ^0"}
       passeStr := ParamStr(passeStr,NumEnString(numeroPasse),'','','');
     end;
 end; {case}
 GetIndString(s,TextesRetrogradeID,2); {'analyse rétrograde de la partie"}
 ...
 if (nbPartiesActives = 1) & JoueursEtTournoisEnMemoire & (windowListeOpen | windowStatOpen) then
   begin  
     ...
     if GetPartieEnAlphaDapresListe(tableNumeroReference^^[1],nbreCoup) <> PartiePourPressePapier(true,false,nbreCoup)
       then   { la partie courante est une intervesion d'une partie de la liste ! On prefere analyser cette derniere }
         begin
...
var nbBlanc,nbNoir,prof : SInt32;
   meiDef,numeroDuCoup,profondeurMilieu : SInt32;
...
begin
  ...
  DessineCourbe(nbreCoup-2,nbreCoup+2,kCourbeColoree,'Amelioration');
  
  SetPhasePartieRetrograde(typeDeJeuDemande);
  vaDepasserTemps := false;
  RefleSurTempsJoueur := false;
  Superviseur(numeroDuCoup);
  if not(calculPrepHeurisFait) then Initialise_table_heuristique(jeuCourant);
    ...
    then
      begin
        ...
        Amelioration := ( choixX <> coupABattre ) &
                        ( scoreParfait > scoreABattre+200 );            {200 = un pion}
      end
    else
      begin
        if debuggage.algoDeFinale then 
			    begin
			      ...
			      WritelnStringAndNumDansRapport('coup = ',nbBlanc+nbNoir-4+1);
			      WritelnStringAndNumDansRapport('scoreABattre = ',scoreABattre);
			      WritelnDansRapport('');
			    end;
			  ...
			  if ((analyseRetrograde.genreAnalyseEnCours=ReflRetrogradeParfait) & (scoreABattre=64)) |
			    ...
			    then
			      ...
			      begin
			        ...
			        (*
			        if useFinaleOld then  { comparatif de vitesse… }
			          begin
					        tickComparaisonAlgosFinales := TickCount();
			        ...
			        *)
			        ...
			        gagnant := CoupGagnant(choixX,meiDef,aQuiDeJouer,prof,nbBlanc,nbNoir,GetCurrentNode(),jeuCourant,emplJouable,
			                             frontiereCourante,HandleFinale,scoreParfait,false,(scoreABattre<-64),analyseRetrograde.genreAnalyseEnCours);
			        if scoreParfait = ScorePourUnSeulCoupLegal then
			           scoreParfait := scoreABattre;
			           
			        (*
			        if useFinaleOld & (interruptionReflexion = pasdinterruption) then  { comparatif de vitesse… }
			          begin
			          
			            tickComparaisonAlgosFinales := TickCount() - tickComparaisonAlgosFinales;
					        FinRapport;
					        TextNormalDansRapport;
					        WriteStringAndNumDansRapport('Nouvel algo (coup ',numeroDuCoup+1);
					        WriteStringAndNumDansRapport(') :  temps = ',tickComparaisonAlgosFinales div 60);
					        ...
					        prof := 64-nbBlanc-nbNoir;
					        bidbool := CoupGagnant(choixX,meiDef,aQuiDeJouer,prof,nbBlanc,nbNoir,GetCurrentNode(),jeuCourant,emplJouable,
					                             frontiereCourante,HandleFinale,scoreParfait,false,(scoreABattre<-64),analyseRetrograde.genreAnalyseEnCours);
					        if scoreParfait=ScorePourUnSeulCoupLegal then
					          scoreParfait := scoreABattre;
					        ...
					        { afficher les resultats du comparatif }
					        if (interruptionReflexion = pasdinterruption) then
					          begin
							        FinRapport;
							        TextNormalDansRapport;
							        WriteStringAndNumDansRapport('Ancien algo (coup ',numeroDuCoup+1);
							        WriteStringAndNumDansRapport(') :  temps = ',tickComparaisonAlgosFinales div 60);
							        WritelnDansRapport(' sec.');
							        WritelnDansRapport('');
							      end;
			  ...
			  if debuggage.algoDeFinale then 
			    begin
			      ...
			      WritelnStringAndNumDansRapport('coup = ',nbBlanc+nbNoir-4+1);
			      WritelnStringAndNumDansRapport('scoreParfait = ',scoreParfait);
  ...
  if not(gameOver) 
    then
      begin
        ...
        GetIndString(s,TextesRetrogradeID,1);  {###################################}
        ...
        scoreOptimalEnChaine := TPCopy(s2,Length(s2)-4,5);
        EnleveEspacesDeGaucheSurPlace(s2);
        if (interruptionReflexion = pasdinterruption)
          then s2 := s1+StringOf('.')+s2
          else s2 := '?????';
        NumToString(nbreCoup,s1);
        GetIndString(s,TextesRetrogradeID,3);
        s := ParamStr(s,s1,s2,'','');
        GetCurrentScript(oldScript);
        WritelnDansRapportSync(s,false);
        GetIndString(s,TextesRetrogradeID,1);  {###################################}
      ...
      begin
        ...
        GetIndString(s,TextesRetrogradeID,4);
...
begin
  ...
  GetIndString(s1,TextesRetrogradeID,5);  {'analyse retrograde en cours'}
  {s1 := Concat(chr(13),s1);}
  if FindStringInRapport(s1,1,1,posChaineAnalyseEnCours)
    then
      begin
        SelectionnerTexteDansRapport(posChaineAnalyseEnCours,posChaineAnalyseEnCours+Length(s1)+1);
...
begin
  if analyseRetrograde.enCours then
    begin
		  GetCurrentScript(oldScript);
		  DisableKeyboardScriptSwitch;
		  GetIndString(s,TextesRetrogradeID,6);   {'analyse rétrograde interrompue…'}
...
begin
  if analyseRetrograde.enCours then
    begin
		  GetCurrentScript(oldScript);
		  DisableKeyboardScriptSwitch;
		  GetIndString(s,TextesRetrogradeID,7);  {'analyse retrograde terminée'}
		  EffacerAnnonceAnalyseRetrogradeEnCours;
		  EffaceDernierCaractereDuRapportSync(false);
		  FinRapport;
		  TextNormalDansRapport;
...
begin
  if analyseRetrograde.enCours & not(analyseRetrograde.dejaAnnoncee) then
    begin
		  GetCurrentScript(oldScript);
		  DisableKeyboardScriptSwitch;
		  GetIndString(s,TextesRetrogradeID,5);  {'analyse retrograde en cours'}
		  EffaceDernierCaractereDuRapportSync(false);
		  FinRapport;
		  TextNormalDansRapport;
		  ChangeFontFaceDansRapport(bold);
...
procedure EcritCoupVerifieAnalyseRetrograde(numeroCoupVerifie,coupVerifie : SInt32; var coupVerifieString : str255);
var s : str255;
    oldScript : SInt32;
begin
  GetCurrentScript(oldScript);
  ...
  NumToString(numeroCoupVerifie,s);
...
procedure EcritCoupFautifDansRapport(numeroCoupVerifie,scorePourNoir,scorePourTrait,nouveauScore,nouveauCoup,nbPionsPerdus : SInt32; var chaineMeilleureSuite,coupVerifieString : str255);
var s,s1,s2 : str255;
    oldScript,i : SInt32;
begin
  s1 := MeilleureSuiteInfosEnChaine(1,true,true,CassioUtiliseDesMajuscules,false,0);
  scoreOptimalEnChaine := TPCopy(s1,Length(s1)-4,5);
                          
  if ((scorePourTrait<0) & (nouveauscore>=0)) |
     ((scorePourTrait=0) & (nouveauscore>0))
    then GetIndString(s,TextesRetrogradeID,9)  {??}
    else GetIndString(s,TextesRetrogradeID,8); {?}
  s := s+'  ';
    ...
    ReflRetrogradeParfait,ReflRetrogradeGagnant:
      begin
        if (scorePourTrait=0) & (nouveauscore>0) then 
			    if aQuiDeJouer = pionNoir
			      then GetIndString(s1,TextesRetrogradeID,10)       {Noir rate le gain}
			      else GetIndString(s1,TextesRetrogradeID,15) else  {Blanc rate le gain}
			  if (scorePourTrait<0) & (nouveauscore>0) then 
			    if aQuiDeJouer = pionNoir
			      then GetIndString(s1,TextesRetrogradeID,11)       {Noir donne le gain}
			      else GetIndString(s1,TextesRetrogradeID,16) else  {Blanc donne le gain}
			  if (scorePourTrait<0) & (nouveauscore=0) then 
			    if aQuiDeJouer = pionNoir
			      then GetIndString(s1,TextesRetrogradeID,12)       {Noir rate la nulle}
			      else GetIndString(s1,TextesRetrogradeID,17)       {Blanc rate la nulle}
			    else 
			      begin
			        if nbPionsPerdus>1
			          then 
			            begin
			              NumToString(nbPionsPerdus,s2);
			              if aQuiDeJouer = pionNoir
			                then GetIndString(s1,TextesRetrogradeID,14)  {Noir perd ^0 pions}
			                else GetIndString(s1,TextesRetrogradeID,19); {Blanc perd ^0 pions}
			              s1 := ParamStr(s1,s2,'','','');
			            end
			          else 
			            if aQuiDeJouer = pionNoir
			              then GetIndString(s1,TextesRetrogradeID,13)  {Noir perd 1 pion}
			              else GetIndString(s1,TextesRetrogradeID,18); {Blanc perd 1 pion}
			      end;
			end;
    ReflRetrogradeMilieu  :
      begin
        if aQuiDeJouer = pionNoir
			    then GetIndString(s1,TextesRetrogradeID,33)  {Noir perd environ ^0 pions}
			    else GetIndString(s1,TextesRetrogradeID,34); {Blanc perd environ ^0 pions}
  ...
  for i := 1 to Length(coupVerifieString) do FrappeClavierDansRapport(chr(RetourArriereKey));
  ...
  NumToString(numeroCoupVerifie,s2);
  ...
  case analyseRetrograde.genreAnalyseEnCours of
    ReflRetrogradeParfait :
      begin
        if scorePourNoir=0
          then GetIndString(s,TextesRetrogradeID,22) {'^0 faisait nulle'}
          else GetIndString(s,TextesRetrogradeID,23);{'^0 faisait ^1'}
    ...
    ReflRetrogradeGagnant :
      begin
        if scorePourNoir=0
          then GetIndString(s,TextesRetrogradeID,22)  {'^0 faisait nulle'}
          else GetIndString(s,TextesRetrogradeID,24); {'^0 était gagnant'}
    ...
    ReflRetrogradeMilieu  :
      begin
        GetIndString(s,TextesRetrogradeID,35); {'^0 était meilleur'}
        s := ParamStr(s,s2+StringOf('.')+CoupEnString(nouveauCoup,CassioUtiliseDesMajuscules),'','','');
        TextNormalDansRapport;
        ChangeFontSizeDansRapport(gCassioRapportNormalSize);
        InsereStringDansRapport(', '+s);
...
begin
  case analyseRetrograde.genreAnalyseEnCours of
    ReflRetrogradeParfait :
      begin
        if scorePourNoir=0
          then GetIndString(s,TextesRetrogradeID,20)  {'fait nulle'}
          else GetIndString(s,TextesRetrogradeID,21); {'fait ^0}
    ...
    ReflRetrogradeGagnant :
      begin
        if scorePourNoir=0 then GetIndString(s,TextesRetrogradeID,20) else {'fait nulle'}
        if scorePourNoir>0 then GetIndString(s,TextesRetrogradeID,25) else {'Noir est gagnant'}
        if scorePourNoir<0 then GetIndString(s,TextesRetrogradeID,26);     {'Blanc est gagnant'}
        GetCurrentScript(oldScript);
        PositionnePointDinsertion(positionInsertionLocale);
        ChangeFontSizeDansRapport(gCassioRapportNormalSize);
        InsereStringDansRapport('   '+s);
    ...
    ReflRetrogradeMilieu :
      begin
        if scorePourNoir>=0
          then 
            begin
              GetIndString(s,TextesSolitairesID,19);    {'Noir'}
              s := Concat('   ',s,NoteEnString(scorePourNoir,true,0,2));
            end
          else 
            begin
              GetIndString(s,TextesSolitairesID,20);   {'Blanc'}
              s := Concat('   ',s,NoteEnString(-scorePourNoir,true,0,2));
            end;
...
var aux:MessageFinaleHdl;
begin
  aux := NIL;
  aux := MessageFinaleHdl(AllocateMemoryHdl(10));
  if aux <> NIL then
    begin
...
begin
  ...
  DessineCourbe(0,nbreCoup,kCourbeColoree,'NettoieToutApresAnalyseRetrograde');
...
var handleRetrograde:MessageFinaleHdl;
    ...
    coupVerifieString,chaineMeilleureSuite : str255;
        ...
        if analyseRetrograde.menuItems[nroPasseAnalyse,1,kMenuGenre]<>RienDuToutCmd then
          begin
            analyseRetrograde.enCours := true;
		            ...
		            nroStageAnalyse := 1;
			          ...
			          numeroCoupVerifie := nbreCoup;
				        ...
				        repeat
				          ...
				          WritelnStringAndNumDansRapport('nroPasseAnalyse=',nroPasseAnalyse);
				          WritelnStringAndNumDansRapport('nroStageAnalyse=',nroStageAnalyse);
				          WritelnDansRapport('');}
				          ...
				          WritelnStringAndNumDansRapport('analyseRetrograde.nroPasseAnalyse=',analyseRetrograde.numeroPasse);
				          WritelnStringAndNumDansRapport('analyseRetrograde.numeroStage=',analyseRetrograde.numeroStage);
				          WritelnStringAndNumDansRapport('analyseRetrograde.genreAnalyseEnCours=',analyseRetrograde.genreAnalyseEnCours);}
			              ...
			              repeat
			                analyseRetrograde.peutDemanderConfirmerArret := true;
				              ...
				              if PeutCopierEndgameScoreFromGameTreeDansCourbe(GetCurrentNode(),nbreCoup,[kFinaleParfaite,kFinaleWLD]) then {do nothing};
					            ...
					            {WriteStringAndNumDansRapport('numeroCoupVerifie=',numeroCoupVerifie);
					            WritelnStringAndNumDansRapport(' , nbreCoup=',nbreCoup);}
					            ...
					            if (interruptionReflexion <> pasdinterruption)
                                  ...
                                  { SysBeep(0);
					                ...
					                begin
					                  ...
					                  AjoutePropertyValeurExacteCoupDansCurrentNode(analyseRetrograde.genreAnalyseEnCours,scoreOptimalPourNoir);
					                  coupPrecedentTermine := true;
					                  dec(numeroCoupVerifie);
					                end;
					              
					            MDisposeHandle(Handle(handleRetrograde));
			 ...
			 {
			  WritelnStringAndNumDansRapport('temps de cette passe = ',(TickCount() - analyseRetrograde.tickDebutCettePasseAnalyse) div 60);
	     }
	      EffacerAnnonceAnalyseRetrogradeEnCours;
        inc(nroPasseAnalyse);
...
begin
  ...
  if FindStringInRapport(s1, positionClic - 2, -1, pos1) & 
     FindStringInRapport(s2, positionClic-Length(s2)+1, +1, pos2)
    then 
      begin
...
begin
  ...
  GetIndString(s1,TextesRetrogradeID,1);  {'###################'}
  ...
  if FindStringInRapport(s1,positionClic,-1,pos1) then
   begin
     ...
     GetIndString(s2,TextesRetrogradeID,5);  {analyse retrograde en cours…}
     if FindStringInRapport(s2,positionClic,+1,pos2) then 
       posFin := Min(posFin,pos2+Length(s2));
     
     GetIndString(s2,TextesRetrogradeID,6);  {analyse retrograde interrompue…}
     if FindStringInRapport(s2,positionClic,+1,pos2) then 
       posFin := Min(posFin,pos2+Length(s2));
     
     GetIndString(s2,TextesRetrogradeID,7);  {analyse retrograde terminée…}
     if FindStringInRapport(s2,positionClic,+1,pos2) then 
       posFin := Min(posFin,pos2+Length(s2));
