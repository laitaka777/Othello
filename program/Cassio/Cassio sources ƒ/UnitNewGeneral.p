...
INTERFACE
 ...
 uses UnitOth2,UnitMacExtras,UnitBlocsDeCoin; 


 const TailleReserveePourLaPile      = 150000;
 
 
 var TailleReserveePourLesSegments : SInt32;
...
USES UnitArbreDeJeuCourant,UnitNouveauFormat,UnitHashTableExacte,UnitListe,UnitStatistiques,UnitRapport;
...
var uneTaille,uneTailleInteger,uneTailleLongint : SInt32;
  ...
  procedure RapporteErreurEtQuitte(s : str255;x,y : SInt32);
    begin {$UNUSED s,x,y}
...
begin
  ...
  Unetaille := nbParties+1;        { sizeof(unsignedbyte) = 1 octet }
  ...
  (*
  unetaille := nbParties+1;
  unetaille := sizeofstr60*unetaille;    { sizeof(str60) = 62 octets }
...
begin
  ...
  WritelnStringAndNumDansRapport('   mémoire totale (en octets) : ',memoire);
  WritelnStringAndNumDansRapport('   mémoire totale (en kilos) : ',memoire div 1024);
  
  memoire := GetEspaceDisponibleLancementCassio();
  WritelnStringAndNumDansRapport('   mémoire disponible au lancement de Cassio (en octets) : ',memoire);
  WritelnStringAndNumDansRapport('   mémoire disponible au lancement de Cassio (en kilos) : ',memoire div 1024);
  
  grow := 0;
  memoire := FreeMem() - TailleReserveePourLesSegments;
  WritelnStringAndNumDansRapport('   mémoire disponible maintenant (en octets) : ',memoire);
  WritelnStringAndNumDansRapport('   mémoire disponible maintenant (en kilos) : ',memoire div 1024);
  
  grow := 0;
  memoire := MaxMem(grow)-TailleReserveePourLesSegments;
  WritelnStringAndNumDansRapport('   plus gros bloc initialisable maintenant (en octets) : ',memoire);
  WritelnStringAndNumDansRapport('   plus gros bloc initialisable maintenant (en kilos) : ',memoire div 1024);
  
  memoire := TailleReserveePourLesSegments;
  WritelnStringAndNumDansRapport('   taille réservée pour les segments (en octets) : ',memoire);
  WritelnStringAndNumDansRapport('   taille réservée pour les segments (en kilos) : ',memoire div 1024);
  
  memoire := nbTablesHashExactes * (sizeof(CoupsLegauxHashArray) + sizeof(HashTableExacteRec));
  WritelnStringAndNumDansRapport('   nbre de tables de hachage : ',nbTablesHashExactes);
  WritelnStringAndNumDansRapport('   mémoire utilisée par les tables de hachage (en octets) : ',memoire);
  WritelnStringAndNumDansRapport('   mémoire utilisée par les tables de hachage (en kilos) : ',memoire div 1024);
  
  memoire := nbrePartiesEnMemoire * kTailleNecessaireParPartie;
  WritelnStringAndNumDansRapport('   nbre parties chargeables pour la base WThor: ',nbrePartiesEnMemoire);
  WritelnStringAndNumDansRapport('   mémoire utilisée par la base (en octets) : ',memoire);
  WritelnStringAndNumDansRapport('   mémoire utilisée par la base (en kilos) : ',memoire div 1024);
...
procedure NewParamDiag(var paramDiag:ParamDiagRec);
begin
  paramDiag.CommentPositionFFORUM := str120Hdl(AllocateMemoryHdl(sizeof(str120)));
  paramDiag.TitreFFORUM := str120Hdl(AllocateMemoryHdl(sizeof(str120)));
...
begin
  ...
  *)
  ...
  PageImpr.TitreImpression := str255Hdl(AllocateMemoryHdl(sizeof(str255)));
  titrePartie := str255Hdl(AllocateMemoryHdl(sizeof(str255)));
  partie := partieHdl(AllocateMemoryHdl(sizeof(t_partie)));
  CheminAccesThorDBA := str255Hdl(AllocateMemoryHdl(sizeof(str255)));
  CheminAccesThorDBASolitaire := str255Hdl(AllocateMemoryHdl(sizeof(str255)));
  CheminAccesSolitaireCassio := str255Hdl(AllocateMemoryHdl(sizeof(str255)));
  for j := 1 to NbMaxItemsReouvrirMenu do
    begin
      nomDuFichierAReouvrir[j] := str255Hdl(AllocateMemoryHdl(sizeof(str255)));
      nomLongDuFichierAReouvrir[j] := str255Hdl(AllocateMemoryHdl(sizeof(str255)));
    end;
  ParametresOuvrirThor := tableauStr255Hdl(AllocateMemoryHdl(sizeof(tableauStr255)));
  CommentaireSolitaire := str255Hdl(AllocateMemoryHdl(sizeof(str255)));
  meilleureSuiteStr := str255Hdl(AllocateMemoryHdl(sizeof(str255)));
  ...
  derniereChaineComplementation := str255Hdl(AllocateMemoryHdl(sizeof(str255)));
  ...
  if gIsRunningUnderMacOSX
    ...
    else nbrePartiesEnMemoire := CalculeNbrePartiesOptimum(MaxMem(grow)-TailleReserveePourLesSegments);
...
begin
  ...
  *)
  ...
  TerminateMenu(GetAppleMenu(),true);
  TerminateMenu(GetFileMenu(),true);
