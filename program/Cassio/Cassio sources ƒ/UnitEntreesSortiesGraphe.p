...
USES UnitAccesStructuresGraphe,StringTypes,UnitDefinitionsPackedThorGame;
...
procedure AjoutePartieDansGrapheApprentissage(var fichier : Graphe;partieStr : str255);

{Verifications et statistiques diverses}

function RepareListeDesFreres(var fichier : Graphe;numCellule : SInt32; var listeDesFreresReparee : ListeDeCellules; const fonctionAppelante : str255) : OSErr;
function RepareOrbite(var fichier : Graphe;numCellule : SInt32; var orbiteReparee : ListeDeCellules; const fonctionAppelante : str255) : OSErr;
procedure VerifieIntegriteCellule(var fichier : Graphe; var cell : CelluleRec;numeroCellule : SInt32; const fonctionAppelante : str255);
...
procedure NettoyerGrapheApprentissage(nomDuGraphe : str255);
...
function TrouveCoupDansGrapheApprentissage(partieStr:str120; var coup : SInt16) : boolean;
...
function CheminDepuisRacineGrapheEnAlphanumerique(var fichier : Graphe;numCellule : SInt32) : str255;
...
USES UnitActions,UnitCalculsApprentissage,UnitOth1,UnitOth2,UnitScannerOthellistique,UnitRapport,UnitJeu,
     MyStrings,MyMathUtils,UnitMacExtras,SNStrings,UnitFenetres,UnitNormalisation,UnitPositionEtTrait,UnitPackedThorGame;
...
begin
		  ...
		  if (orbite.cardinal >= kLongueurListeInfinie) then
		    begin
		      ...
		      for i := 1 to Min(kLongueurListeInfinie,20) do
		        begin
		          WriteStringAndNumDansRapport('->',orbite.liste[i].numeroCellule);
		      ...
		      for i := 1 to Min(kLongueurListeInfinie,20) do
		        begin
		          WriteStringAndNumDansRapport('->(',orbite.liste[i].numeroCoup);
		          WriteStringAndNumDansRapport(',',orbite.liste[i].fils);
		          ...
		          begin
		            ...
		            if (bonneOrbite.cardinal > 0) then
		              begin
				            ...
				            for k := 1 to bonneOrbite.cardinal do
				              begin
				                WriteStringAndNumDansRapport('cell #',bonneOrbite.liste[k].numeroCellule);
				                WritelnStringAndNumDansRapport(' -> memePosition=',bonneOrbite.liste[1 + (k mod bonneOrbite.cardinal)].numeroCellule);
		      ...
		      LitCellule(fichier,numCellule,cellule);
		      SetEcritToutDansRapportLog(true);
		      VerifieIntegriteCellule(fichier,cellule,numCellule,Concat(fonctionAppelante , 'RepareOrbite : '));
		    end;
		end;
  ...
  WritelnStringAndNumDansRapport('Sortie de RepareOrbite, fonctionAppelante = '+fonctionAppelante+'  ==> orbiteReparee.cardinal = ',orbiteReparee.cardinal);
...
begin
      ...
      aux := premierFrere;
		  ...
		  if (freres.cardinal >= kLongueurListeInfinie) then
		    begin
		      ...
		      for i := 1 to Min(kLongueurListeInfinie,20) do
		        begin
		          WriteStringAndNumDansRapport('->',freres.liste[i].numeroCellule);
		          if (i mod 10) = 0 then WritelnDansRapport('');
		        end;
		      ...
		      for i := 1 to Min(kLongueurListeInfinie,20) do
		        begin
		          WriteStringAndNumDansRapport('->(',freres.liste[i].numeroCoup);
		          WriteStringAndNumDansRapport(',',freres.liste[i].pere);
		          WriteStringDansRapport(')');
		          if (i mod 10) = 0 then WritelnDansRapport('');
		        end;
		          ...
		          begin
		            ...
		            if (bonFreres.cardinal > 0) then
		              begin
				            ...
				            for k := 1 to bonFreres.cardinal do
				              begin
				                WriteStringAndNumDansRapport('cell #',bonFreres.liste[k].numeroCellule);
				                WritelnStringAndNumDansRapport(' -> frere=',bonFreres.liste[1 + (k mod bonFreres.cardinal)].numeroCellule);
		      ...
		      LitCellule(fichier,numCellule,cellule);
		      SetEcritToutDansRapportLog(true);
		      VerifieIntegriteCellule(fichier,cellule,numCellule,Concat(fonctionAppelante , 'RepareListeDesFreres : '));
		    end;
		end;
  ...
  WritelnStringAndNumDansRapport('Sortie de RepareListeDesFreres, fonctionAppelante = '+fonctionAppelante+'  ==> listeDesFreresReparee.cardinal = ',listeDesFreresReparee.cardinal);
        ...
        begin
          ...
          if (coupCourant<11) | (coupCourant>88) then
              begin
                ...
                WritelnStringAndNumDansRapport('coupCourant = ',coupCourant); 
              end;
...
begin
  ...
  with cell do
       ...
       if (Ensemble.cardinal >= kLongueurListeInfinie) | (aux<=0)
         then
           begin
             ...
             WritelnStringAndNumDansRapport('aux = ',aux);
             {AttendFrappeClavier;}
                    ...
                    if Ensemble.cardinal>0 then
                      begin
                        ...
                        if not(ok) then
                          begin
                            AnnonceProblemeDansRapport;
                            WritelnDansRapport(StringOf('#')+NumEnString(numeroCellule)+' : frères mais pas même père…');
                            {AttendFrappeClavier;}
                            for k := 1 to Min(10,Ensemble.cardinal) do
                              begin
                                LitCellule(fichier,Ensemble.liste[k].numeroCellule,cellAux);
   ...
   if probleme then
     begin
       WritelnStringAndNumDansRapport('Un pb a ete trouve pour la cellule #',numeroCellule);
          ...
          repeat
            ...
            if (coupCherche<11) | (coupCherche>88) then
              begin
                ...
                WritelnStringAndNumDansRapport('coupCherche = ',coupCherche); 
              end;
...
var filsJouables:listeDeProbas;
    sommeProbas,alea : extended;
    i : SInt16; 
    SommesPartielles : array[0..64] of extended;
begin
          ...
          begin
            ...
            if HasFils(cell) then
              begin
                ...
                LitEnsembleDesFils(fichier,path.liste[path.cardinal].numeroCellule,FilsConnus);
                {WritelnStringAndNumDansRapport('FilsConnus.cardinal=',FilsConnus.cardinal);
                for i := 1 to FilsConnus.cardinal do
                  WritelnStringAndNumDansRapport('FilsConnus['+NumEnString(i)+']=',FilsConnus.liste[i].numeroCellule);}
                ...
                {WritelnStringAndNumDansRapport('FilsAffiches.cardinal=',FilsAffiches.cardinal);}
                ...
                if avecFlecheProchainCoupDansGraphe then
                  begin
                    PenPat(whitePattern);PenSize(1,1);
