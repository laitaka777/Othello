...
USES Palettes, QDOffScreen, UnitMacExtras;
...
USES UnitRapport,UnitServicesDialogs;
...
{That makes the code more readable.}
	...
	type
		ByteArr = packed array[0..400000] of Byte;
		ByteArrPtr = ^ByteArr;
{ Global rectangles.}
	var
...
(* assume that min is less than max *)
	begin
		qdRdm := BitAnd(SInt32(Random()), $0000ffff);		{Avoid negative values while keeping the full range.}
		range := max - min;
...
{ Init the Macintosh environment}
	begin
		...
		GetFNum('Old English Text',oldEnglishID);
	...
	{ Set Up Offscreen GWorld}
		...
		if CampFireErr <> 0 then
			begin
			  AlerteSimple('Pas assez de mŽmoire pour Campfire !!');
			  AttendFrappeClavier;
			  ExitToShell;
			end;
	...
	{ Set a pionter to the first byte of the ofscreen PixMap}
		...
		CampFireRowByte := BitAnd(OffscreenPixMapH^^.rowBytes, $3fff);
...
(*******************************)
procedure WriteTextInFire(const s : str255;x,y : SInt32);
var color : SInt32;
begin
  ...
  {WritelnStringAndNumDansRapport('',color);}
  ...
  ForeColor(RedColor);
	...
	TextFont(GenevaID);
	TextSize(18);
	DrawString('  '+NumEnString(color));
	DrawString('  '+NumEnString(seuilAllumage));
	}
