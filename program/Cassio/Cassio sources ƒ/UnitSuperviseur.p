...
procedure Initialise_table_heuristique(jeu : plateauOthello);
...
USES UnitUtilitaires,UnitOth1,UnitStrategie,UnitRapport,UnitMacExtras,MyStrings,UnitGestionDuTemps;
...
var i,t : SInt16; 
	...
	begin
	  ...
	  if CaseDebutLigne+dx*(LongueurLigne-1) <> CaseFinLigne then 
	    begin
	      SysBeep(0);
	      WritelnStringAndNumdansRapport('erreur dans le pattern : ',numeroPattern);
...
begin
  ...
  *)
  ...
  for i := 1 to 64 do
   ...
   for j := 1 to 4 do
   begin
      StringToNum(TPCopy(tableau[j],5*i-4,4),valeur);
      deb := valeur div 1000;
      fin := valeur mod 100;      
      t := i*2+10*j*2;
      dirPriseDeb[t] := deb;
  ...
  for i := -1 to 60 do 
    begin
      WriteStringAndNumAt('info[',i,10+(i div 30)*240,(i mod 30)*10+20);
      WriteStringAndNumAt(']=',IndexInfoDejaCalculeesCoupNro^^[i],70+(i div 30)*240,(i mod 30)*10+20);
    end;
  AttendFrappeClavier;
  }
		  ...
		  for i := 1 to 8 do
		    begin
		      for j := 1 to 8 do
			      begin
			        t := 10*j+i;
			        WriteStringAndNumDansRapport('(',nroPattern[t,compteur]);
			        WriteStringAndNumDansRapport(',',deltaAdresse[t,compteur]);
			        WriteStringAndNumDansRapport(',',doubleDeltaAdresse[t,compteur]);
			        WriteDansRapport(') ');
			      end;
			    WritelnDansRapport('');
			  end;
 ...
 for i := 1 to 8 do
   begin
     for j := 1 to 8 do
	     begin
	       t := 10*j+i;
	       WriteStringAndNumDansRapport('',nbPatternsImpliques[t]);
	     end;
	   WritelnDansRapport('');
	 end;
 WritelnDansRapport('');
...
begin
   ...
   for j := 1 to 4 do
   begin
      StringToNum(TPCopy(tableau[i],5*j-4,4),valeur);
...
procedure Calcul_position_centre(plat : plateauOthello);
var Ysomme,Xsomme : extended;
    SigmaPoids,poids : extended;
    x1,y1,x2,y2 : SInt16; 
    i,x : SInt16; 
    caseOccupee : boolean;
begin
 ...
 for i := 11 to 88 do
   ...
   for j := 1 to 8 do
     ...
     begin
       a := 100+i*25;
       b := 50+j*15;
       if plat[i+10*j] = pionNoir then WriteStringAndNumAt(CaracterePourNoir,0,a,b);
       if plat[i+10*j] = pionBlanc then WriteStringAndNumAt(CaracterePourBlanc,0,a,b);
       if plat[i+10*j] = pionVide then WriteStringAndNumAt('',valeurtact[i+10*j],a,b);
     end;   
   AttendFrappeClavier;
   }
...
var i,j,dist,x,compt,m,t : SInt16; 
    tableDeTri : array[1..14,1..14] of SignedByte;
...
begin
  ...
  {WritelnStringAndNumDansRapport('temps de Initialise_table_heuristique = ',ticks);}
end;


function Elagage_a_priori(couleur,nbBla,nbNoi : SInt16; jeu : plateauOthello; var class : ListOfMoveRecords;longClass : SInt16) : SInt16; 
var PourcentageElagage : extended ;
    i,t : SInt16; 
    index : SInt16; 
