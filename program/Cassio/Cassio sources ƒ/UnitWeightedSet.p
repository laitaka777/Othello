...
procedure WriteWeightedSetDansRapport(const nom : str255;S:WeightedSet;avecCardinal : boolean);
procedure WritelnWeightedSetDansRapport(const nom : str255;S:WeightedSet;avecCardinal : boolean);
...
begin
  ...
  if WeightedSetEstVide(S)
    then WriteStringAndNumDansRapport('{ }, cardinal=',S.cardinal)
    else
      begin
        WriteDansRapport('{');
        L := S.theSet;
        repeat
          ...
          WriteStringAndNumDansRapport('(',element);
          WriteStringAndNumDansRapport(',',poids);
          WriteDansRapport(')');
          L := L^.tail;
          if (L <> NIL) then WriteDansRapport(' ');
        until (L = NIL);
        WriteStringDansRapport('}');
        if avecCardinal then WriteStringAndNumDansRapport(', cardinal=',S.cardinal);
        if avecCardinal then WriteStringAndNumDansRapport(', somme=',S.somme);
      end;
end;
