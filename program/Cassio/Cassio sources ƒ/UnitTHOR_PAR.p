...
USES UnitNouveauFormat,UnitFichiersTEXT;


type ThorParRec =
       packed record
                 texte                        : packed array[1..36] of char;      {36 octets}
                 posEtCoups                   : packed array[1..8,1..8] of byte;  {64 octets}
                 tempRestant                  : SInt16;                           {2 octets}
                 modeDeJeu                    : SInt16;                           {2 octets}
                 niveauDansThor               : byte;                             {1 octet}
                 nbCoupsJouesYComprisPasses   : byte;                             {1 octet}
                 nbNoirs                      : byte;                             {1 octet}
                 nbBlancs                     : byte;                             {1 octet}
                 traitInitial                 : byte;                             {1 octet}
                 traitPosFinale               : byte;                             {1 octet}
...
function GetCommentOfThorParRec(const enregistrement : ThorParRec) : str255;
function GetPartieEnAlphaOfThorParRec(const enregistrement : ThorParRec) : str255;
...
USES UnitScannerOthellistique,UnitAccesStructuresNouvFormat,UnitUtilitaires,UnitOth1,UnitServicesDialogs,
     UnitDialog,UnitBaseNouveauFormat, UnitTriListe,UnitListe,UnitRapport,MyStrings,UnitEntreeTranscript,
     UnitImportDesNoms,UnitEntreesSortiesListe,UnitRapportImplementation,SNStrings,UnitCouleur,UnitPackedThorGame;
...
var err : OSErr;
    ...
    commentaire,partieEnAlpha : str255;
        ...
        with enregistrement do
          if (nbCoupsJouesYComprisPasses > 0) then
            begin
              ...
              if (Length(partieEnAlpha) div 2) > 20 then
                begin
...
var err : OSErr;
    k,i,j,aux : SInt16; 
    s,joueurNoir,joueurBlanc : str255;
  ...
  if (numeroPartieDansFichier >= 0) & 
    ...
    begin
      ...
      {les 36 pemiers octets sont du texte libre}
		  ...
		  with partieNF do
		    begin
		      ...
		      UpperString(joueurNoir,false);
		      UpperString(joueurBlanc,false);
		      for k := 1 to Min(14,Length(joueurNoir)) do  s := s+joueurNoir[k];
		      s := s+' '+ScoreFinalEnChaine((scoreReel-32)*2)+' ';
		      for k := 1 to Min(14,Length(joueurBlanc)) do s := s+joueurBlanc[k];
		    end;
		  {on complete à 36 caracteres avec le caractere nul}
		  if Length(s)>36
		    then s := TPCopy(s,1,36)
		    else for k := length(s)+1 to 36 do s := s+chr(0);
      ...
      for k := 1 to 60 do
		  ...
		  if Length(s)>TailleEnregistrementTHOR_PAR
		    then s := TPCopy(s,1,TailleEnregistrementTHOR_PAR)
		    else for k := length(s)+1 to TailleEnregistrementTHOR_PAR do s := s+chr(0);
	        ...
	        begin
	          ...
	          {if (nbPartiesEcrites mod 500) = 0 then
	            begin
	              ...
	              WritelnStringAndNumDansRapport(' ,  refusees : ',nbPartiesRefusees);
	            end;}
	        end
	      else
	        begin
...
begin
  ...
  if windowListeOpen & (nbPartiesActives>500) then
    begin
      GetIndString(prompt,TextesNouveauFormatID,7); {'Impossible de sauvegarder plus de 500 parties dans un fichier THOR.PAR !'}
      AlerteSimple(prompt);
      SauvegardeListeCouranteEnTHOR_PAR := NoErr;
      exit(SauvegardeListeCouranteEnTHOR_PAR);
    end;
  ...
  BeginDialog;
  GetIndString(s,TextesNouveauFormatID,6); {'THOR.PAR'}
  reply.fname := s;
  GetIndString(prompt,TextesNouveauFormatID,5);   {'nom du fichier THOR.PAR à créer ?'}
