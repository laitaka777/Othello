...
procedure CalculerRect3D(x : SInt16);
...
procedure Dessine3D(var position : plateauOthello;avecBruitage : boolean);
...
USES UnitOth1,UnitGeometrie,UnitCassioSounds,UnitBufferedPICT,UnitStrategie,
     Unit3DPovRayPicts,UnitCouleur;
...
var a,b : SInt32;
    dxHaut,dxBas,x1Ext,x2Ext : extended;
    i,j : SInt16; 
    x1,x2,y1,y2,x3,x4,y3,y4 : SInt32;
  begin
        ...
        then
          ...
          begin
            SetValeurs3DParDefaut(xbasDeb,xhautDeb,xBasFin,xHautFin,ybas,yhaut,Xcentre);
			      ...
			      PenPat(blackPattern);
			      if avecDessinFondNoir then EraseRect(unRect);
			  ...
			  case coul of
			    ...
			    pionSuggestionDeCassio:
			        begin
			          ...
			          {InsetRect(unRect,0,1);}
			          inc(unRect.top);inc(unRect.top);dec(unRect.bottom);
...
begin
  ...
  compteurSons := 0;
  if avecBruitage & avecSonPourPosePion then PlayPosePionSound; {PlaySoundSynchrone(sonPourPosePion);}
...
begin
  ...
  if EnVieille3D()
    then
      ...
      begin
        for x := 1 to 64 do
          if PtInRect(loc,GetBoundingRect3D(othellier[x])) then
            begin
