...
{$DEFINEC CASSIO_DEBUG_TRACE true }
{$ENDC }
...
procedure WriteInTraceLog(s : str255);
...
USES TextEdit,UnitOth0,UnitRapportImplementation,UnitServicesDialogs,SNStrings,UnitOth2,
     UnitServicesRapport,UnitCarbonisation;
...
begin
  ...
  if erreurES=fnfErr then {-43 => fichier non trouvé, on le crée}
    begin
      ...
      SetFileCreatorFichierTexte(FichierTraceLog,'R*ch');  {BBEdit}
      SetFileTypeFichierTexte(FichierTraceLog,'TEXT');
    end;
