...
USES UnitOth0,UnitSquareSet,UnitDefinitionsProperties,MacTypes;
...
function MakeRealProperty(whichType : SInt16; whichReal : extended) : Property;
function MakeStringProperty(whichType : SInt16; WhichString : str255) : Property;
...
function MakeSeptCaracteresProperty(whichType : SInt16; whichSquare : SInt16; whichCaracteres:str7) : Property;
...
function MakeQuintupletProperty(whichType : SInt16; whichLong : SInt32;b0,b1,b2,b3 : SignedByte) : Property;


{fonctions d'acces aux infos des proprietes sur les types de base}
function GetLongintInfoOfProperty(prop : Property) : SInt32;
function GetRealInfoOfProperty(prop : Property) : extended;
function GetStringInfoOfProperty(prop : Property) : str255;
...
procedure GetSquareAndSeptCaracteresOfProperty(prop : Property; var square : SInt16; var SeptChar:str7);
...
procedure GetQuintupletOfProperty(prop : Property; var theLong : SInt32; var b0,b1,b2,b3 : SignedByte);


{fonctions de changement de l'info d'une propriete}
procedure SetLongintInfoOfProperty(var prop : Property;n : SInt32);
procedure SetRealInfoOfProperty(var prop : Property;r : extended);
procedure SetStringInfoOfProperty(var prop : Property;s : str255);
...
procedure SetSquareAndSeptCaracteresOfProperty(var prop : Property;whichSquare : SInt16; whichCaracteres:str7);
...
procedure AddStringToTexteProperty(var prop : Property;s : str255);
...
procedure SetQuintupletOfProperty(var prop : Property;whichLong : SInt32;b0,b1,b2,b3 : SignedByte);
...
procedure WriteStringAndPropertyDansRapport(s : str255; var prop : Property);
procedure WritelnStringAndPropertyDansRapport(s : str255; var prop : Property);
procedure WritelnSoldesCreationsPropertiesDansRapport(prefixe : str255);

{fonctions de traductions}
function StringToPropertyGenre(s : str255) : SInt16; 
function PropertyTypeToString(whichType : SInt16) : string;
function PropertyValueToString(prop : Property) : str255;
function PropertyToString(prop : Property) : str255;
...
type PropertyLocalisation =
     ...
     InfoQuintuplet = 
       packed record
         theLong : SInt32;
         theSignedBytes : packed array[0..3] of signedByte;
       end;
...
USES UnitStrategie,UnitOth1,UnitOth2,UnitRapport,UnitNormalisation,UnitScannerOthellistique,
     SNStrings,UnitServicesMemoire,UnitMacExtras,UnitCouleur;
...
procedure WritelnSoldesCreationsPropertiesDansRapport(prefixe : str255);
begin
  WritelnStringAndNumDansRapport(prefixe+'SoldeCreationProperties = ',SoldeCreationProperties);
  WritelnStringAndNumDansRapport(prefixe+'SoldeCreationPropertyList = ',SoldeCreationPropertyList);
  WritelnStringAndNumDansRapport(prefixe+'SoldeCreationGameTree = ',SoldeCreationGameTree);
  WritelnStringAndNumDansRapport(prefixe+'SoldeCreationGameTreeList = ',SoldeCreationGameTreeList); 
end;
...
begin
  ...
  WritelnStringAndNumDansRapport('   => solde = ',SoldeCreationProperties);}
...
begin
  r := whichReal;
  MakeRealProperty := MakeProperty(whichType,sizeof(extended),@whichReal,StockageEnReal);
end;
...
begin
  if WhichString=''
    then
      ...
      begin
        whichTaille := Length(WhichString)+1;
        MakeStringProperty := MakeProperty(whichType,whichTaille,@WhichString,StockageEnStr255);
      end;
end;
...
var len,i : SInt16; 
    aux : Property;
begin
  len := Length(whichCaracteres);
  if len>7 then len := 7;
  for i := len+1 to 7 do whichCaracteres[i] := chr(0);
  whichCaracteres[0] := chr(whichSquare);
  
  
  with aux do
    begin
      MoveMemory(@whichCaracteres,@taille,8);  {attention : type casting}
      genre := whichType;
      stockage := StockageEnSeptCaracteres;
    end;
...
begin
  ...
  {
  WriteStringAndNumDansRapport('signe = ',whichSign);
  WriteStringAndNumDansRapport(', int = ',whichIntegerValue);
  WriteStringAndNumDansRapport(', cent = ',centiemes);
...
begin
  ...
  if ((scorePourNoir < -64) | (scorePourNoir > 64)) & 
     not(GenreDeReflexionInSet(quelGenreDeReflexion,[ReflMilieu,ReflRetrogradeMilieu,ReflMilieuExhaustif,ReflZebraBookEval])) then
    begin
      SysBeep(0);
      WritelnDansRapport('ERREUR : scorePourNoir = '+NumEnString(scorePourNoir)+' dans MakeScoringProperty(ReflParfait), prévenez Stéphane !');
    ...
    ReflParfait,ReflRetrogradeParfait,ReflParfaitExhaustif :
      begin
        ...
        if odd(scorePourNoir) then
          begin
            Sysbeep(0);
            WritelnStringAndNumDansRapport('BIZARRE : score impair dans MakeScoringProperty {1}, scorePourNoir = ',scorePourNoir);
          end;
        
        if (scorePourNoir > 64) | (scorePourNoir < -64) then
          begin
            Sysbeep(0);
            WritelnStringAndNumDansRapport('ASSERT : score out of range dans MakeScoringProperty {2}, scorePourNoir = ',scorePourNoir);
          end;
    ...
    ReflGagnant,ReflRetrogradeGagnant,ReflParfaitExhaustPhaseGagnant, ReflGagnantExhaustif:
      begin
        ...
        if (scorePourNoir > 64) | (scorePourNoir < -64) then
          begin
            Sysbeep(0);
            WritelnStringAndNumDansRapport('ASSERT : score out of range dans MakeScoringProperty {4}, scorePourNoir = ',scorePourNoir);
          end;
      end;
...
function MakeQuintupletProperty(whichType : SInt16; whichLong : SInt32;b0,b1,b2,b3 : SignedByte);
  ...
  with theQuintuplet do
    begin
      ...
      with aux do
		  ...
		  inc(SoldeCreationProperties);
		  WritelnStringAndNumDansRapport('sizeof(theQuintuplet) = ',sizeof(theQuintuplet));
		  WriteStringAndPropertyDansRapport('creation de ',aux);
      WritelnStringAndNumDansRapport('   => solde = ',SoldeCreationProperties);  
    end;
end;
...
begin
  ...
  if prop.info <> NIL then
    begin
      s := '';
      longueur := prop.taille-1;
      if longueur<0 then longueur := 0;
      if longueur>255 then longueur := 255;
      if (longueur>0) then
        begin
          MoveMemory(prop.info,@s[0],prop.taille);
          s[0] := chr(longueur);
        end;
...
begin
  if (prop.stockage = StockageEnSeptCaracteres) 
    then
      begin
        MoveMemory(@prop.taille,@aux,8);   {type casting}
        square := LENGTH_OF_STRING(aux);
        SeptChar := '';
        for i := 1 to 7 do
          if aux[i]<>chr(0) then SeptChar := Concat(SeptChar,aux[i]);
      end
    else
      begin
        square := 0;
      ...
      if s=''
        then
          ...
          begin
            taille := Length(s)+1;
            info  := AllocateMemoryPtrClear(((taille+3) div 4)*4);
			      if info <> NIL then MoveMemory(@s,info,taille);
			    end;
    end;
end;
...
procedure SetOthelloSquareOfPropertyAlpha(var prop : Property;coup : SInt16);
begin
  SetStringInfoOfProperty(prop,CoupEnStringEnMinuscules(coup));
end;
...
begin
  if (prop.stockage=StockageEnSeptCaracteres) then
    begin
      ...
      for i := len+1 to 7 do whichCaracteres[i] := chr(0);
      whichCaracteres[0] := chr(whichSquare);
      MoveMemory(@whichCaracteres,@prop.taille,8);    {type casting}
    end;
end;
          ...
          begin
            ...
            if newSpace <> NIL then
              begin
                ...
                MoveMemory(texte,MakeMemoryPointer(ord4(SInt32(newSpace)+oldTaille)),longueur);
                DisposeMemoryPtr(prop.info);
                prop.info := newSpace;
...
procedure AddStringToTexteProperty(var prop : Property;s : str255);
begin
  AddTexteToProperty(prop,@s[1],Length(s));
end;
...
procedure SetRectangleAffichageOfProperty(var prop : Property;whicRect : rect);
type PropertyLocalisationPtr= ^PropertyLocalisation;
var infosProperty:PropertyLocalisationPtr;
begin
  if (prop.stockage = StockageEnPtrProperty) & (prop.info <> NIL) 
    then
	    begin
	      infosProperty := PropertyLocalisationPtr(prop.info);
	      infosProperty^.rectAffichage := whicRect;
	    end;
end;
...
begin
   {
   WritelnStringAndNumDansRapport('prop.genre = ',prop.genre);
   WritelnStringAndNumDansRapport('prop.stockage = ',prop.stockage);
   WritelnStringAndNumDansRapport('prop.taille = ',prop.taille);
   WritelnStringAndNumDansRapport('prop.infos = ',SInt32(prop.info));
   }
...
begin
  ...
  {propriétés definies par Cassio, better way}
  if s='DW'    then StringToPropertyGenre := DeltaWhiteProp                else
  if s='DB'    then StringToPropertyGenre := DeltaBlackProp                else
