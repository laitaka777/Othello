...
procedure EssaieAjouterInterversionPotentielle(alpha1,alpha2 : str255;whichNode : GameTree);

{trouver tous les chemins}
function ChercheToutesInterversionsPositionEtTrait(var whichPos : PositionEtTraitRec;coupUn,refGoal : SInt32;whichGame : stringPtr;metDansGraphe,dansRapport,sousIntervertions : boolean) : SInt32;
procedure ChercheToutesInterversionsPartie(var partieAlpha : str255;coupUn,refPartie : SInt32;metDansGraphe,sousIntervertions : boolean; var nbInter : SInt32);
...
{Diverses}
procedure TraiteInterversionFormatThorCompile(var s60 : PackedThorGame);
procedure TraiteInterversionFormatThor(var s60 : PackedThorGame; longueur : SInt16);
procedure PrecompileInterversions(var s60 : PackedThorGame; longueur : SInt16);
procedure TraiteIntervertionsCoups(var s:str120);
procedure InterversionPuisNormalisation(var partie120:str120; var autreCoupQuatreDiag : boolean);
procedure AjouterInterversion(variante,canonique:str120);
...
USES UnitFichiersTEXT,UnitRapport,UnitRapportImplementation,UnitOth2,UnitJaponais,UnitOth1,
     UnitPositionEtTraitSet,UnitPileEtFile,UnitDefinitionsSmartGameBoard,UnitGameTree,
     UnitStrategie,UnitApprentissageInterversion,UnitArbreDeJeuCourant,UnitAfficheArbreDeJeuCourant,
     UnitJeu,UnitGeneralSort,UnitDialog,MyFileSystemUtils,UnitServicesDialogs,UnitMiniProfiler,
     UnitHashing,MyStrings,UnitScannerOthellistique,SNStrings,UnitNormalisation,UnitEntreeTranscript,
     UnitPackedThorGame;
...
begin
  ...
  if MakeFileName(reply,'Nom du fichier d''interversions ?',mySpec) then
      begin
        ...
        nomfichier := reply.fName+StringOf('1');
        ...
        erreurES := FermeFichierTexte(fichierInterversions);
        SetFileCreatorFichierTexte(fichierInterversions,'CWIE'); 
        SetFileTypeFichierTexte(fichierInterversions,'TEXT');
        
        
        nomfichier := reply.fName+StringOf('2');
        erreurES := FichierTexteExisteFSp(MyMakeFSSpec(mySpec.vRefNum,mySpec.parID,nomfichier),fichierInterversions);
        if erreurES=fnfErr then erreurES := CreeFichierTexteFSp(MyMakeFSSpec(mySpec.vRefNum,mySpec.parID,nomfichier),fichierInterversions);
        if erreurES=0 then
          begin
...
begin
  ...
  {
  WriteStringAndNumAt('temps en ticks : ',(TickCount()-chrono),100,150);
  AttendFrappeClavier;
  for i := 0 to 20 do
    begin
      NumToString(i,s);
      WriteStringAndNumAt('inter longueur '+s+' : ',numeroInterversion[i],10,10+11*i);
    end;
  AttendFrappeClavier;
  for i := 21 to 33 do
    begin
      NumToString(i,s);
      WriteStringAndNumAt('inter longueur '+s+' : ',numeroInterversion[i],10,10+11*(i-21));
    end;
  AttendFrappeClavier;
  WriteStringAndNumAt('nb interversions : ',numeroInterversion[33],100,100);
  AttendFrappeClavier;
  }
...
procedure AjouterInterversionAlaVolee(InterVarianteAlaVolee,InterCanonAlaVolee : PackedThorGame; longueur : SInt16; whichNode : GameTree);
var variante120,canonique120:str120;
    canonique255,variante255 : str255;
    canoniqueAff,varianteAff : str255;
...
begin
  if (longueur <= 33) & (longueur >= 5) then
    begin
      ...
      AddTranspositionPropertyToGameTree(variante255,whichNode);
            ...
            begin
              ...
              varianteAff := TPCopy(variante120,1,2*t-2)+StringOf('-')+TPCopy(variante120,2*t-1,2*longueur-2*t+2);
		          ...
		          if not(PendantLectureFormatSmartGameBoard()) & not(EnModeEntreeTranscript()) then
		            begin
		              ParamText(canoniqueAff,varianteAff,'','');
...
procedure EssaieAjouterInterversionPotentielle(alpha1,alpha2 : str255;whichNode : GameTree);
var partie1_120,partie2_120:str120;
    s1,s2 : PackedThorGame; 
    autreCoupQuatre : boolean;
begin
  if Length(alpha1) = Length(alpha2) then
    begin
    ...
    begin
      ...
      if ABRSearch(impasses.arbre,posHash) <> NIL
		    then
		      begin
		        if trace then
		          WritelnStringAndNumDansRapport('Deja dans impasses : ',GenericHash(@whichPos,sizeof(whichPos)));
		      end
		    else
			    begin
			      ...
			      {WritelnStringAndNumDansRapport('NbElementsDansPile = ',NbElementsDansPile(pileIndexImpasses));}
			      
			      if trace then
			        WritelnStringAndNumDansRapport('Empiler '+NumEnString(posHash)+' / ',goalHash);
      ...
      repeat
        ...
        if (elementTrouve <> NIL) & (elementTrouve^.data=goalHash) then
          begin
            ...
            if trace then
              WritelnStringAndNumDansRapport('Depiler '+NumEnString(posHash)+' / ',goalHash);
...
function ChercheToutesInterversionsPositionEtTrait(var goal : PositionEtTraitRec;coupUn,refGoal : SInt32;whichGame : StringPtr;metDansGraphe,dansRapport,sousIntervertions : boolean) : SInt32;
const kCaseRemplieCouleurIndeterminee = 35;
var couleurPionsForces : platValeur;
    i,j,t,nbCoupMax,intervaleExtreme : SInt32;
	          ...
	          begin
	            if SamePositionEtTrait(goal,posAux) then
                  ...
                  *)
                  ...
                  if dansRapport {& not(estTriviale)} {& (longueurInterversionPrincipale=numeroCoup)} then
	                  begin
	                    WriteStringAndNumDansRapport('#',nbLigneAffichee);
	                    ...
	                    for i := numeroCoup+1 to longueurInterversionPrincipale do
	                      WriteDansRapport('  ');
	                    WriteStringAndNumDansRapport('    ( ',nbSousInter);
	                    WriteStringAndNumDansRapport(' , #',nbLigneAffichee);
	                    WriteStringAndNumDansRapport(' , ->#',refGoal);
            ...
            begin 
              ...
              if MemberOfPositionEtTraitSet(posAux,goalHash,impasses)
                then
                  begin
                    {if trace then
                      WritelnStringAndNumDansRapport('Impasse utilisee : goalHash ='+NumEnString(goalHash)+'  vraieHash = ',GenericHash(@goal,Sizeof(goal)));
                    }
                  ...
                  begin
			                    ...
			                    if trace then
			                      begin
			                        ...
			                        WritelnDansRapport(' recursif ');
									        ...
									        if chercheSolutionsLocales 
									          then
									            begin
									              ...
									              for dist := intervaleExtreme downto 1 do
									                begin
									                  x := goalLigne[numeroCoup+1]+dist;
									                  {WritelnStringAndNumDansRapport(' dist = ',dist);
									                  WritelnStringAndNumDansRapport(' test de x = ',x);}
			                          ...
			                          {WritelnStringAndNumDansRapport(' dist = ',0);
			                          WritelnStringAndNumDansRapport(' test de x = ',x);}
											          if (x>=caseNordOuest) & (x<=caseSudEst) & (posAux.position[x] = pionVide) then
											            begin
											              couleurCase := couleurPionsForces[x];
			                      ...
			                      begin
			                        WriteDansRapport('  goal='+whichGame^+' ');
			                          ...
			                          WriteDansRapport(CoupEnStringEnMajuscules(ligne[t]));
									            WritelnStringAndNumDansRapport('  , ',nbFaconAtteindrePosCherchee);
									          end;
									        *)
			        ...
			        {WritelnStringAndNumDansRapport('nbFaconAtteindrePosCherchee=',nbFaconAtteindrePosCherchee);}
    ...
    then  {oui, cool}
      begin
        if trace then
		      begin
		        if whichGame <> NIL
              then WritelnStringAndNumDansRapport('cool '+ whichGame^+' direct : ',compteurReussites)
              else WritelnStringAndNumDansRapport('cool direct : ',compteurReussites);
          end;
        ...
        MemoryFillChar(@couleurPionsForces,sizeof(couleurPionsForces),chr(0));
			  ...
			  if whichGame <> NIL then
			    begin
			      ...
			      if chercheSolutionsLocales 
			        then intervaleExtreme := (caseSudEst-caseNordOuest);
			    end;
			  {WritelnStringAndNumDansRapport('intervaleExtreme = ',intervaleExtreme);
              ...
              then WritelnDansRapport('ChercheToutesInterversionsPositionEtTrait '+whichGame^+' recursif')
				...
				JouerCoup(PositionEtTraitInitiauxStandard(),coupUn,1);
				
			  {
			  WritelnStringAndNumDansRapport('compteurReussites = ',compteurReussites);
				WritelnStringAndNumDansRapport('nbPositionsVisitees = ',nbPositionsVisitees);
				WritelnStringAndNumDansRapport('memoisationEnAvant.cardinal = ',memoisationEnAvant.cardinal);
				WritelnStringAndNumDansRapport('hauteur de  memoisationEnAvant= ',ABRHauteur(memoisationEnAvant.arbre));
        }
				            ...
				            if trace then
						          if MemberOfPositionEtTraitSet(positionForcee,infoTableGlobale,memoisationInterversionsEnArriere)
								            ...
								            WriteDansRapport('  goal='+whichGame^+' ');
														...
														WriteStringAndNumDansRapport('  , ',compteurReussites);
				...
				if trace then
				  if MemberOfPositionEtTraitSet(goal,infoTableGlobale,memoisationInterversionsEnArriere)
		            ...
		            WriteDansRapport('  goal='+whichGame^+' ');
							  WriteStringDansRapport(' : dans la table globale -> ');
							  WriteDansRapport(whichGame^);
							  WritelnStringAndNumDansRapport('  , ',compteurReussites);
							end;
				*)
...
var s60 : PackedThorGame; 
    ...
    sousPartie : str255;
begin
  if (Length(partieAlpha)>0) & EstUnePartieOthello(partieAlpha,true) then
    begin
      if trace then
      ...
      {WritelnStringAndNumDansRapport('nb interversions de '+partieAlpha+' = ',nbInter);}
    end;
end;


procedure EcritSystemeMinimalInterversionsDansRapport(partieAlpha : str255;coupUn : SInt32;metDansGraphe : boolean; var nbInter : SInt32);
begin
  VideMemoisationInterversions;
  ViderImpasses;
  longueurInterversionPrincipale := Length(partieAlpha) div 2;
  ...
  WritelnDansRapport('');
  WritelnStringAndNumDansRapport('Nombre interversions = ',nbInter);
  WritelnStringAndNumDansRapport('Taille du systeme minimum d''interversions = ',nbAffichageSousLignesDansRapport);
  WritelnStringAndNumDansRapport('Nombre de coups joues = ',nbAppelsJouerCoup);
 {WritelnStringAndNumDansRapport('impassesLocales.tete = ',impassesLocales.tete);
  WritelnStringAndNumDansRapport('impassesLocales.queue = ',impassesLocales.queue);}
  WritelnStringAndNumDansRapport('Nombre positions memorisees = ',NbPositionsMemoizees());
  WritelnStringAndNumDansRapport('Hauteur de l''arbre binaire de recherche= ',ABRHauteur(memoisationInterversionsEnArriere.arbre));
  WritelnDansRapport('');
...
{  tick := TickCount()-tick;
  WritelnStringAndNumDansRapport('temps utilise = ',tick);}
        			        ...
        			        begin
        			          if SamePositionEtTrait(positionVerification,position)
          		              ...
          		              begin
          		                ...
          		                {WritelnStringAndNumDansRapport('nb Collisions Interversions = ',GetNbCollisionsInterversions());}
          		              end
          		        end
        			      else
        			        begin
        			          WritelnDansRapport('WARNING : not(GetPositionEtTraitACeNoeud(noeudDansTable,positionVerification))');
        			        end;
        			  end
      			  else
      			    begin
...
begin
  if (table <> NIL) then
    begin
      WritelnStringAndNumDansRapport('orbite.cardinal = ',table^.cardinal);
		  for i := 1 to table^.cardinal do
		    begin
...
begin
  ...
  if CalculeCycleTrieDesInterversions(whichNode,tableLignes,index) &
     GetPositionEtTraitACeNoeud(whichNode,whichPosition) then
    begin
      ...
      longueur := Length(s) div 2;
      if SamePositionEtTrait(whichPosition,PositionEtTraitAfterMoveNumberAlpha(s,longueur,typeErreur))
        then 
          begin
            ...
            if GetPositionEtTraitACeNoeud(gameNodeLePlusProfond,positionTestee) &
...
begin
  ...
  if CalculeCycleTrieDesInterversions(whichNode,tableLignes,index) &
     GetPositionEtTraitACeNoeud(whichNode,position) then
    begin
...
begin
  ...
  for i := NbinterversionsCompatibles downto 1 do
    begin
      ...
      longueurEnCours := Length(compare);
...
begin
  ...
  if debuggage.general then
    begin
      ...
      WriteStringAndNumAt(s255,0,10,120);
    end;
  }
  ...
  if debuggage.general then
    begin
      ...
      WriteStringAndNumAt(s255,0,10,132);
    end;
  }
  ...
  for nroInterversion := numeroInterversion[longueur] downto 1 do
    begin
      ...
      longueurEnCours := Length(canonique);
  ...
  {WritelnStringAndNumDansRapport('NbinterversionsCompatibles = ',NbinterversionsCompatibles);}
  
  {
  t := 0;
  for i := 3 to 33 do
    begin
      WriteStringAndNumDansRapport('numeroInterversion[',i);
      WritelnStringAndNumDansRapport('] = ',numeroInterversion[i]);
...
begin
  nbrecoupDansPartie := Length(partie120) div 2;
...
begin
  ...
  if (longueur <= 33) then
                 ...
                 begin
                   if (fautive60 = interversionCanonique^^[i]) then
                     begin
                       ...
                       SysBeep(0);
                       WriteStringAndNumAt('mauvais ordre !!! '+ variante+'  ',GET_LENGTH_OF_PACKED_GAME(fautivePacked),10,100);
                       AttendFrappeClavier;
                     end;
                 end;
             end;
    ...
    begin
      WriteStringAndNumAt('ligne trop longue !!!  '+variante, longueur,10,50);
      SysBeep(0);
      dejaRentree := true;
    end;
end;
